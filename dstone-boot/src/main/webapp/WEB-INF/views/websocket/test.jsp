				...
				<div class="inner">
					...
					<section>
					    ...
					    <script>
					    	...
					    	const socket = new WebSocket("<%=scheme.equals("http")?"ws":"wss"%>://<%=requestUtil.getServerName()%>:<%=requestUtil.getServerPort()%><%=net.dstone.boot.common.config.ConfigWebSocket.WEBSOCKET_WS_END_POINT%>");
					    ...
					    <script>
					        ...
					        function stompClientconnect() {
					            const socket = new SockJS("<%=scheme%>://<%=requestUtil.getServerName()%>:<%=requestUtil.getServerPort()%><%=net.dstone.boot.common.config.ConfigWebSocket.WEBSOCKET_STOMP_END_POINT%>");
					            stompClient = Stomp.over(socket);
					            stompClient.connect({}, function () {
					                stompClient.subscribe("<%=net.dstone.boot.common.config.ConfigWebSocket.WEBSOCKET_STOMP_SUB_PREFIX%>/message", function (msg) {
					        ...
					        function sendStompMessage() {
					            ...
					            stompClient.send("<%=net.dstone.boot.common.config.ConfigWebSocket.WEBSOCKET_STOMP_PUB_PREFIX%>/message", {}, JSON.stringify({
					                sender: sender,
					                content: content
					            }));
					        }
