...
public class OrderSagaReplyListener extends BaseObject {
	...
	/**
	 * <pre>
	 * step01-inventoryReserve 스텝의 결과 이벤트를 수신해 step02-payment 스텝을 트리거한다.
	 * payload 파라메터는 Kafka ConsumerRecord.value()가 역직렬화된 것 그대로다.
	 * 흐름: 
	 *   SagaOrchestrator.runStep()이 step01-inventoryReserve 처리 후
	 *   outboxAppender.append("step01-inventoryReserve-reply", sagaId, result) 호출 
	 *   OutboxRelay가 topic="step01-inventoryReserve-reply", key=sagaId, value=result(JSON)로 실제 발행한 메시지다.
	 *   value 안에는 Step01InventoryReserveService.handle()이 반환한 원래 command 필드(ORDER_ID, ITEM_ID, QTY, AMOUNT)에 엔진이 주입한 "SAGA_ID" 필드가 합쳐져 들어있다.
	 * </pre>
	...
	/**
	 * <pre>
	 * step02-payment 스텝의 결과 이벤트를 수신해 step03-orderConfirm 스텝을 트리거한다.
	 * payload는 "step02-payment-reply" 토픽 메시지의 value로, 
	 * Step02PaymentService.handle()의 반환값(command 원본 + SAGA_ID)이 그대로 역직렬화된 것이다.
	 * </pre>
	 * @param payload Kafka 메시지 value(JSON→Map). "SAGA_ID" 키로 사가 식별자를 꺼내 쓴다.
	 */
	...
	/**
	 * <pre>
	 * step03-orderConfirm 스텝(사가의 마지막 스텝)의 결과 이벤트를 수신해 사가를 COMPLETED로 종결한다.
	 * 이 사가 정의에서 "step03-orderConfirm 이 마지막 스텝"이라는 사실은 오케스트레이터가 아니라
