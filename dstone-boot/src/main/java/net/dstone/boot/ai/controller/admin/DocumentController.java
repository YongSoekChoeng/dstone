...
import net.dstone.boot.ai.service.admin.DocumentService;
...
/**
 * RAG 문서 임베딩 적재/목록/삭제를 관리하는 컨트롤러다. 개발자가 찔러보는 "테스트"가 아니라 실제로 벡터스토어에 데이터를
 * 넣고 빼는 관리 작업이라 controller.admin 패키지, /ai/admin/document/* URL에 둔다. 전부 AJAX라
 * ChatController와 같은 이유로 @RestController로 둔다.
 * 파일업로드는 이 코드베이스의 기존 관례대로 RequestUtil/FileUpUtil(FILE_UPLOAD_ROOT에 저장)을 그대로 쓴다.
 */
@RestController
@RequestMapping("/ai/admin/document/*")
