 ...
 * 실제 Kafka 전송은 append()가 리턴한 뒤, 별도 스레드의 OutboxRelay.dispatchPending()이
 * TB_OUTBOX_MESSAGE를 폴링하면서 비동기로 수행한다. 이렇게 "DB 커밋"과 "메시지 발행 성공"을
 * 시간적으로 분리함으로써, DB는 커밋됐는데 Kafka 전송에 실패해 이벤트가 유실되는 문제
 * (혹은 그 반대의 이중 쓰기 문제)를 없앤다 — 최소 한 번(at-least-once) 발행을 보장한다.
 * </pre>
 */
public interface OutboxAppender {
	 ...
	 * @param key     ProducerRecord의 key(메시지 키)에 매핑된다. ConfigKafka에 설정된 StringSerializer로
	 *                직렬화된다. Kafka 프로듀서는 이 key를 해시(hash(key) % 파티션수)해서 파티션을 결정하므로,
	 *                "같은 key를 가진 메시지는 항상 같은 파티션에 순서대로 쌓인다"는 성질을 이용해
