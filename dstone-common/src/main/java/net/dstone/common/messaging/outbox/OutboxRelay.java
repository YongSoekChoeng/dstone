 ...
 *
 * OutboxAppender.append()가 "DB에 예약"만 해두면, 
 * 이 클래스가 별도 스레드에서 그 예약 건들을 실제 Kafka 메시지로 전송하는 역할을 나눠 맡는다(Producer/Relay 역할 분리). 
 * 이 분리 덕분에 비즈니스 트랜잭션(OutboxAppender.append 호출 시점)이 Kafka 브로커의 가용성이나 네트워크 지연에 영향받지 않는다.
 * </pre>
 */
public class OutboxRelay extends BaseObject {
	...
	/**
	 * <pre>
	 * TB_OUTBOX_MESSAGE에서 PENDING 상태인 레코드를 조회해 Kafka로 발행을 시도하고, 건별로 성공/실패를 DB에 반영한다. 건(row)별 독립 처리.
	 * [row 각 컬럼 → Kafka ProducerRecord 매핑]
	 *  - row.TOPIC    → kafkaTemplate.send(topic, key, payload)의 topic 인자 (전송 대상 토픽)
	 *  - row.MSG_KEY  → kafkaTemplate.send(topic, key, payload)의 key 인자 (파티션 결정용, StringSerializer로 직렬화)
	 *  - row.PAYLOAD  → kafkaTemplate.send(topic, key, payload)의 key payload. 
	 *                   DB에는 JSON 문자열로 저장돼 있던 것을 objectMapper로 Map<String,Object>로
	 ...
	 *              기본 100건씩, 1초(spring.kafka.messaging.outbox.relay-interval-ms) 간격으로 이 메소드를 호출한다.
	...
	public int dispatchPending(int limit) {
		...
		for (Map<String, Object> row : rows) {
			...
			try {
				...
				// Kafka 로 먼저 전송하고 끝난 이후 DB에 상태(SENDING)반영을 한다.(순서 중요)
	...
	/**
	 * <pre>
	 * SENDING 상태로 staleSeconds 이상 방치된 행(claimPending()으로 선점됐지만 markSent()/markFailed()반영 전에 프로세스가 죽는 등
	 * 완료되지 못한 행)을 다시 PENDING으로 되돌려 다음 dispatchPending() 폴링에서 재시도되게 한다. 
	 * dstone-boot의 OutboxRelayScheduler가 dispatchPending()보다 훨씬 긴 주기로 별도 호출한다.
	 * </pre>
