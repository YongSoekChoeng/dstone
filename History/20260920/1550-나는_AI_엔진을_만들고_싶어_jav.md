...
1. AGENT 스텝이 inputTemplate을 아예 무시합니다 (가장 큰 제약) ==>> 이 내용의 뜻이뭘까 ?
...
`StepDefinition`에는 `inputTemplate`이라는 필드가 있습니다. 이름 그대로 "이 스텝에 넘길 입력을 어떻게 조립할지" 정하는 템플릿인데, 실제로는 **TOOL 스텝에서만 쓰이고 AGENT/SUPERVISOR 스텝에서는 아예 읽지도 않습니다.**

## 코드로 보면

TOOL 스텝(`ToolStepRunner.java:46`):
```java
String jsonInput = this.renderToolInput(definition.inputTemplate(), normalized, input.variables());
//                                       ^^^^^^^^^^^^^^^^^^^^^^^^^^ inputTemplate을 실제로 읽어서 {previous}, {변수명} 치환
```

AGENT 스텝(`AgentStepRunner.java:47`):
```java
String answer = this.agentExecutor.call(agent, execution.sessionId(), execution.caller(),
    input.variables(), input.renderedText(), null, null, null);
//                                            ^^^^^^^^^^^^^^^^^^^^ StepDefinition을 아예 안 받음 - inputTemplate 볼 방법이 없음
```

그리고 이 `input.renderedText()`가 어디서 만들어지는지 보면(`WorkFlowExecutor.java:212`):
```java
StepInput input = new StepInput(this.previousText(execution), execution.variables());
//                               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ 그냥 "직전 스텝 출력 텍스트" 그대로. definition.inputTemplate()은 여기서도 안 씀
```

즉 AGENT 스텝의 입력 텍스트는 **YAML에 `inputTemplate`을 뭐라고 적든 상관없이 무조건 "바로 앞 스텝의 출력 그대로"**로 고정됩니다.

## 실제로 뭐가 안 되는지 (예시)

`oracle-to-postgresql.yml`의 `validate` 스텝(TOOL)은 이렇게 적혀 있습니다:
```yaml
inputTemplate: '{"sql":"{previous}"}'
```
이건 TOOL이라서 정상 동작합니다.

그런데 만약 AGENT 스텝에서 "직전 스텝 출력"과 "맨 처음 사용자 입력"을 **둘 다** 프롬프트에 넣고 싶어서
```yaml
- id: fix
  type: AGENT
  ref: sql-fix-agent
  inputTemplate: '원본 SQL: {original}\n\n검증 실패 내용: {previous}'
```
이렇게 적어도, `AgentStepRunner`는 이 `inputTemplate` 필드를 아예 참조하지 않으므로 **완전히 무시되고 그냥 `{previous}`(직전 스텝 출력) 텍스트만 프롬프트로 들어갑니다.** YAML 작성자 입장에선 "왜 이 템플릿이 반영이 안 되지?" 하고 헷갈리게 되는 지점입니다.

## 왜 이게 "가장 큰 제약"인지

지금 있는 3개 워크플로우 예제(`oracle-to-postgresql`, `parallel-test`, `sample-approval-flow`)는 전부 "직전 스텝 출력 하나만 그대로 다음 스텝에 넘기면 되는" 단순 릴레이 구조라 이 제약이 안 드러납니다. 하지만 여러 값을 조합해야 하는 업무 — 예: "1단계에서 뽑은 사용자 프로필 + 2단계에서 뽑은 요청 내용을 합쳐서 3단계 LLM에 프롬프트로 준다" — 을 만들려는 순간 AGENT 스텝으로는 표현할 방법이 없다는 뜻입니다. `StepInput` 클래스 주석엔 "AGENT/TOOL 공통으로 쓰는 입력 원문"이라고 적혀 있는데(`StepInput.java`), 실제 코드는 그렇지 않으니 문서와 동작이 어긋나 있는 셈입니다.

---

원하시면 지금 바로 고쳐드릴 수 있습니다. 고치는 방법은 간단합니다: `ToolStepRunner`에만 있는 템플릿 치환 로직(`{previous}`/`{변수명}` → 실제 값)을 `WorkFlowExecutor.runOne()`/`runGroup()` 쪽으로 끌어올려서, TOOL이든 AGENT든 `StepInput`을 만들 때 공통으로 `inputTemplate`을 적용하게 하면 됩니다. `inputTemplate`이 없는 스텝(현재 예제 대부분)은 지금처럼 "직전 출력 그대로"가 기본값으로 유지되니 기존 YAML은 깨지지 않습니다.
