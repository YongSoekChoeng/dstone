...
{
    "timestamp": "2026-09-09T03:23:28.511Z",
    "status": 400,
    "error": "Bad Request",
    "path": "/api/ai/rag/documents"
}
...
좋아, boundary 문제는 해결됐어! 이번 건 다른 원인이야:

```
MissingServletRequestParameterException: Required request parameter 'sourceId' for method parameter type String is not present
```

**`sourceId` 값이 요청에 안 실려서 나는 에러**야. `RagController`가 `sourceId`를 필수 파라미터로 요구하는데(`@RequestParam("sourceId") String`), form-data body에 그 키가 빠졌거나 체크가 꺼져있는 상태로 보내진 것 같아.

## 확인할 것

1. `rag - 문서 업로드` 요청의 **Body 탭** 열기
2. `form-data` 테이블에 두 줄이 있는지 확인:
   - `file` — Type: **File**, 값: 업로드할 파일 선택돼 있는지
   - `sourceId` — Type: **Text**, 값: 예) `sample-doc-1` (비어있지 않은지)
3. 각 줄 맨 왼쪽 **체크박스가 켜져 있는지**(꺼놓으면 그 키 자체가 전송에서 빠짐)
4. `sourceId` 행의 **key 이름이 정확히 `sourceId`**인지(대소문자/오타 확인 — `sourceid`, `source_id` 등으로 되어 있으면 서버가 못 찾음)

`sourceId` 값 채워 넣고 다시 Send 해보고 결과 알려줘.

---

## 질문

3. 단독 검색으로 잘 들어갔는지 확인 에서 "topK":3 은 뭐야 ?

---

## 답변

"Top-K"는 검색/추천 시스템에서 흔히 쓰는 용어로, **"유사도 순으로 정렬했을 때 상위 K개만 가져와라"**는 뜻이야.

## 정확히 뭘 하는 값인가

```json
{"query": "임베딩 모델로 뭘 쓰나요?", "topK": 3, ...}
```

이 요청이 들어오면:
1. `query`를 Ollama로 벡터화
2. pgvector에 저장된 **모든** 문서 청크와 코사인 유사도를 계산
3. 유사도 높은 순으로 정렬
4. **그중 상위 3개만** 잘라서 반환

즉 `topK: 3`이면 아무리 관련 문서가 많아도 딱 3개까지만 받아온다는 뜻이야. `topK: 10`으로 바꾸면 10개까지 받아오고.
...
`topK`를 요청에서 빼거나 `null`로 두면 `dstone.ai.rag.retrieval.top-k`(`application.yml`, 현재 기본값 **5**)가 적용돼.

## `similarityThreshold`와의 관계

둘 다 같이 적용되는 **별개의 필터**야:
- `topK` = 개수 제한 ("최대 몇 개까지")
- `similarityThreshold` = 품질 커트라인 ("유사도가 이 값 이상인 것만")

예를 들어 `topK: 3, similarityThreshold: 0.35`면 **"유사도 0.35 이상인 것들 중에서 가장 비슷한 3개까지"**. 유사도 0.35 넘는 게 1개뿐이면 1개만 나오고, 아예 없으면 빈 배열이 나와(지난번 검색 결과 빈 배열 문제가 이거였지).

## 실무에서 어떻게 고르나

- **너무 작게(1~2)**: RAG-증강 채팅에서 관련 있는 다른 문서 조각을 놓칠 수 있음
- **너무 크게(10+)**: 관계없는 내용까지 프롬프트에 섞여 들어가서 LLM이 헷갈리거나, 컨텍스트 토큰만 낭비(특히 로컬 소형 모델은 컨텍스트 윈도우가 좁아서 더 민감)
- 보통 **3~5** 정도로 시작해서 실제 답변 품질 보고 조정하는 게 일반적이야 — 지금 기본값(5)도 그 범위 안에 있어
...
아래단계 진해하자.
Phase3. Agent/Tool | Function calling, Tool 등록 체계, 단순 오케스트레이션

---

## 답변

Phase 3(Agent/Tool)로 들어가기 전에, Spring AI 2.0.1의 Tool Calling API를 정확히 확인해볼게 (Phase 2 때처럼 추측하지 않고 실제 클래스를 까봐야 정확해).

---

## 답변

Confirmed the Tool Calling API. Now let's design Phase 3 to match the project's established conventions and implement it.

---

## 답변

Now the sample tool, mirroring `dstone-boot`'s `sample/` convention.

---

## 답변

Now update the `agent` package-info and add new ones for `tool`/`tool.sample`.

---

## 답변

Now let's wire `toolsEnabled` into `ChatRequest` and `ChatController`.
...
Clean compile. Now let's rebuild, restart, and test the tool-calling flow live.

---

## 답변

Tool auto-discovered at startup. Now let's test function calling live — ask something the LLM can't know without calling the tool.

---

## 답변

Now let's apply all the updates. First, package structure and roadmap sections.

---

## 답변

Now let's renumber sections 7→13 down to 8→14, and insert the new Phase 3 section as section 7. First, the TOC.

---

## 답변

Now let's renumber the actual section headers (7→8, 8→9, 9→10, 10→11, 11→12, 12→13) and insert the new Phase 3 content before old section 7.

---

## 답변

Now let's renumber the remaining sections and update the API reference table for `toolsEnabled`.

---

## 답변

Now let's update the "다음 단계" content itself to remove Phase 3 (now done) and update the API reference table for `toolsEnabled`.

---

## 답변

Now let's update the API reference (chat endpoint) table to add `toolsEnabled`.

---

## 답변

Now let's fix the two cross-references elsewhere that pointed to the old section numbers.

---

## 답변

Now let's add a `toolsEnabled` field/example to the Postman collection for consistency.

---

## 답변

Now let's update `CLAUDE.md` to reflect Phase 3 completion.

---

## 답변

Phase 3(Agent/Tool) 구현하고 실제로 검증까지 마쳤어. 정리하면:

## 구현한 것

- **`net.dstone.ai.agent.tool.AiTool`** — Tool 등록용 마커 애노테이션(`@Component` 메타애노테이션 포함). 클래스에 이거 하나만 붙이면 자동 등록됨
- **`ToolRegistry`** — 기동 시점에 `@AiTool` 빈들을 스캔해서 `ToolCallbackProvider`로 묶어줌. RAG와 달리 외부 인프라 의존이 없어서(순수 Java 실행) on/off 플래그 없이 항상 켜져 있음 — 등록된 Tool이 없으면 그냥 빈 상태
- **`agent.tool.sample.DateTimeTools`** — 패턴을 보여주는 샘플 Tool(`getCurrentDateTime`), `dstone-boot`의 `sample/` 패키지와 같은 성격
- **`ChatController.toolsEnabled`** — `ragEnabled`와 같은 방식으로 요청에 `toolsEnabled: true`만 넣으면 등록된 모든 Tool을 ChatClient에 연결
- **"단순 오케스트레이션"**은 별도 워크플로우 엔진을 만든 게 아니라, Spring AI의 `ChatClient`가 내장하고 있는 tool-calling 루프(LLM이 Tool 호출 요청 → 실행 → 결과를 다시 LLM에 전달 → 반복)를 그대로 활용하는 것으로 범위를 한정했어

## 실동작 검증

```bash
# Tool 없이 - 모델이 모른다고 답함
"지금 몇 시야?" → "실시간 정보에 접근할 수 없어서..."

# toolsEnabled:true - 실제 Tool 호출해서 정확히 답함
"지금 몇 시야?" → "2026-09-09T13:37:29" (실제 시스템 시간과 1초 이내 일치)
```

기동 로그에도 `dstone-ai-engine agent: 등록된 Tool = getCurrentDateTime`이 찍히는 것까지 확인했어.

## 문서 갱신

`docs/dstone-ai-engine.md`에 Phase 3 절(시퀀스 다이어그램 포함) 신설, 로드맵/패키지 구조/API 레퍼런스 갱신(섹션 번호 밀림에 따른 교차 참조도 다 맞춰놨어), `CLAUDE.md`, Postman 컬렉션(`chat - Tool 사용` 요청 추가)도 같이 업데이트했어.

앱은 계속 떠 있는 상태야. 다음으로 Phase 4(governance/observability) 갈지, 아니면 Tool을 하나 더 추가해볼지(예: RAG 검색 자체를 Tool로 노출해서 LLM이 알아서 검색 여부를 판단하게 하는 "agentic RAG") 알려줘.
...
좋아, 지난번에 제안했던 **"Agentic RAG"** — RAG 검색 자체를 Tool로 노출해서, `ragEnabled`처럼 매번 강제로 검색하는 게 아니라 **LLM이 스스로 "이건 검색이 필요하다"고 판단할 때만 검색하도록** 만들어볼게.

먼저 `@Tool` 메소드 파라미터 관련 API를 확인해볼게.

---

## 답변

Confirmed. Now let's implement it inside the `rag.retrieval` package (since it's fundamentally a RAG capability, just exposed via the Tool mechanism, and must be gated the same way as the rest of RAG).

---

## 답변

Let's compile, rebuild, and restart to verify.

---

## 답변

Now let's ingest a test document and verify the LLM decides on its own when to search.

---

## 답변

완벽하게 검증됐어 — Ollama 로그로 실제 검색 호출 여부까지 교차 확인했어:

## 실동작 검증 (3가지 케이스)

| 질문 | LLM의 판단 | 실제 검색 호출(`/api/embed` 로그) |
|---|---|:---:|
| "ollama-webui-lite Uh-oh 에러 원인이 뭐였어?" (지식베이스에 있는 내용) | 지식베이스 검색 필요 판단 → **정확한 원인+해결법으로 답변** | ✅ 호출됨 |
| "1부터 10까지 더하면?" (일반 상식) | 검색 없이 직접 계산 | — |
| "파이썬 리스트 정렬 방법?" (일반 지식) | 검색 없이 직접 답변, **9초간 새 `/api/embed` 호출 없음(로그로 확인)** | ❌ 호출 안 됨 |

`ragEnabled`(매번 강제 검색)와 다르게, **LLM이 질문 성격을 스스로 판단해서 필요할 때만 검색하는 게 실증됐어** — 이게 Agentic RAG의 핵심이야.

## 구현 요약

- `net.dstone.ai.rag.retrieval.RetrievalTools` — `@AiTool` + `@ConditionalOnProperty(dstone.ai.rag.enabled=true)`, `searchKnowledgeBase(query)` 하나 노출
- `RetrievalService`를 그대로 재사용(같은 topK/threshold 기본값)
- `dstone.ai.rag.enabled=false`면 이 Tool도 같이 안 뜸(RAG 자체가 없으니 자연스러운 처리)

이제 기동 로그엔 두 Tool이 다 찍혀:
```
...
Now let's add a dedicated subsection (7.5) documenting the Agentic RAG tool with its verified test evidence.

---

## 답변

Now let's briefly update `CLAUDE.md`'s Phase 3 paragraph to mention the second tool.

---

## 답변

문서까지 다 정리했어. 앱은 계속 떠 있고, 등록된 Tool은 이제 두 개야: `getCurrentDateTime`, `searchKnowledgeBase`. 다음으로 뭘 더 해볼까?

---

## 질문

net.dstone.ai.agent.tool.AiTool 을 
net.dstone.ai.common.annotation.AiTool 로 리팩터링 해줘

---

## 답변

Now let's update `ToolRegistry.java`.

---

## 답변

Now `DateTimeTools.java`:

---

## 답변

Now `RetrievalTools.java`:

---

## 답변

Now the package-info files referencing `AiTool` via javadoc `{@link}`.

---

## 답변

Clean compile. Now let's rebuild, restart, and re-verify both tools still register correctly.

---

## 답변

Both tools still register correctly after the move. Now let's update the docs referencing the old path.

---

## 답변

Now let's update the package structure diagram in `dstone-ai-engine.md`.

---

## 답변

All references updated. Let's do a final live re-verification and check git status.

---

## 답변

리팩터링 완료했어. `AiTool`을 `net.dstone.ai.common.annotation.AiTool`로 옮기고:

- `net.dstone.ai.common.annotation` 패키지 신설(package-info 포함) — `net.dstone.ai.common`이 이미 "dstone-ai-engine 자체 공통 유틸" 용도로 비어있던 자리라 자연스럽게 들어맞았어
- `ToolRegistry`, `DateTimeTools`, `RetrievalTools`의 import 전부 갱신
- `agent/tool/package-info.java`의 javadoc 링크 경로도 갱신
- `docs/dstone-ai-engine.md`(패키지 구조 트리 + 7.2절 코드 예시), `CLAUDE.md`도 새 경로에 맞춰 갱신(왜 `agent` 밖으로 뺐는지 이유도 같이 적어둠 — `rag.retrieval.RetrievalTools`도 쓰기 때문)

재빌드 후 재기동해서 두 Tool(`getCurrentDateTime`, `searchKnowledgeBase`) 모두 정상 등록/동작 재확인했어(방금 다시 시간 질문 테스트도 정상).

---

## 질문

net.dstone.ai.agent.tool.ToolRegistry 을 
net.dstone.ai.config.ConfigTool 로 리팩터링 해줘
그리고 net.dstone.ai.config.Config 에서 @Import 하도록 리팩터링 해줘.

---

## 답변

Now let's create the new `ConfigTool` in `net.dstone.ai.config`, delete the old file, and wire it up.

---

## 답변

Now let's update `Config.java` to `@Import` it.
...
Clean compile. Now rebuild, restart, and re-verify.

---

## 답변

Confirmed dead. Let's restart.

---

## 답변

Both tools still register correctly through `ConfigTool`. Let's do a final functional test.

---

## 답변

Now let's update the docs (`dstone-ai-engine.md`, `CLAUDE.md`) that reference the old `ToolRegistry` path.

---

## 답변

Now the remaining prose mentions:
...
All references cleared. Final check and cleanup.
...
- `net.dstone.ai.agent.tool.ToolRegistry` → **`net.dstone.ai.config.ConfigTool`**로 이동(클래스명도 변경)
- `net.dstone.ai.config.Config`의 `@Import` 목록에 `ConfigTool.class` 추가 — `dstone-boot`/`dstone-batch`/`dstone-batchadmin`과 동일한 컨벤션대로 다른 `Config*` 클래스들과 나란히
- `ChatController`의 필드/생성자 파라미터도 `configTool`로 갱신
- `AiTool.java`, `agent/package-info.java`, `agent/tool/package-info.java`, `docs/dstone-ai-engine.md`, `CLAUDE.md`의 경로 언급도 전부 갱신

빌드/재기동 후 재검증까지 마쳤어 — 기동 로그에 `등록된 Tool = getCurrentDateTime, searchKnowledgeBase` 그대로 찍히고, 실제 채팅 테스트("지금 몇 시야?")도 정상 동작 확인했어.
...
지난번처럼 전체 `docs/` 디렉토리를 다시 훑어볼게. 이번엔 Phase 3(Agent/Tool)와 그 이후 리팩터링(`AiTool`→`common.annotation`, `ToolRegistry`→`config.ConfigTool`) 관련 내용이 빠짐없이 반영됐는지가 핵심이야 — 상당 부분은 작업하면서 바로 갱신해뒀지만, 놓친 게 있는지 전수 점검할게.

---

## 답변

Let's verify the full current source tree matches exactly what's documented, and validate the TOC/anchors are all consistent after all the incremental edits.

---

## 답변

마지막으로, `dstone-ai-engine.md`의 API 레퍼런스 섹션에 Postman 컬렉션 링크를 추가해서 실제로 바로 써볼 수 있게 연결해둘게.
...
이번엔 지난번(RAG Phase 2 때)만큼 손댈 게 많지 않았어 — Phase 3 작업하면서 그때그때 `docs/dstone-ai-engine.md`(섹션 7 신설, 패키지 구조, 로드맵, API 레퍼런스)와 `CLAUDE.md`를 바로바로 갱신해뒀기 때문이야. 전체 24개 md 파일을 grep으로 훑어서 찾은 실제 갭은 두 가지뿐이었어:

1. **`docs/README.md`** — 모듈 문서 목록에서 `dstone-ai-engine.md` 한 줄 설명이 "Chat/Gateway/Session/Prompt(Phase 0~1), RAG(Phase 2)"까지만 있고 **Phase 3(Agent/Tool)가 빠져있었음** → 추가
2. **`docs/dstone-ai-engine.md`** — API 레퍼런스 섹션에 Postman 컬렉션 링크가 없어서 바로 연결해뒀어

## 재확인한 것들 (문제 없었음)

- 리팩터링 두 건(`AiTool`→`common.annotation`, `ToolRegistry`→`config.ConfigTool`) 관련 옛 경로/클래스명이 문서 어디에도 안 남아있는지 grep으로 전수 검색 → 0건
- 문서의 패키지 구조 트리가 실제 소스 트리(`find ... -name "*.java"`)와 정확히 일치하는지 대조 → 일치
- 목차와 실제 헤딩이 정확히 매칭되는지, 다른 문서에서 `dstone-ai-engine.md#절번호`로 건 링크들이 지난번 섹션 번호 밀림(7→8 등) 이후에도 다 맞는지 재확인 → 전부 정상
- 코드 펜스(``` ```)/`<details>` 태그 짝 맞는지 → 정상

Phase 3는 새 인프라나 빌드/배포 영향이 전혀 없어서(순수 Java 코드) `build.md`/`cloud-architecture.md`/`environment.md`/`docs/software/*.md`는 이번엔 손댈 이유가 없었어 — 그 문서들이 Phase 3를 언급 안 하는 게 정확한 상태야.

---

## 질문

@Tool이 호출되는 메커니즘이 어떻게 되는가?

---

## 답변

정확한 메커니즘을 코드 레벨까지 까봤어. 크게 3단계(등록 → LLM에게 알림 → 실행 루프)로 나뉘어.
...
- `ConfigTool`이 `@AiTool` 빈들을 찾아서, 그 안의 **`public` + `@Tool` 붙은 메소드**를 리플렉션으로 뒤진다.
- 각 메소드마다 `ToolDefinition`을 만드는데, 이때 메소드의 **파라미터 타입/이름(`-parameters` 컴파일 옵션 필요)과 `@ToolParam` 설명**을 보고 **JSON Schema를 자동 생성**한다.
  - `getCurrentDateTime()` → 파라미터 없음 → 빈 스키마
  - `searchKnowledgeBase(@ToolParam(description="검색할 질문 또는 키워드") String query)` → `{"query": {"type":"string", "description":"검색할 질문 또는 키워드"}}` 같은 스키마

## 2단계: 채팅 요청 시 — LLM에게 "이런 도구들이 있다"고 알려줌

`ChatController`가 `.toolCallbacks(configTool.toolCallbackProvider())`로 붙이면, 이 요청이 Claude API로 나갈 때 **`tools` 파라미터에 방금 만든 ToolDefinition 목록(이름/설명/스키마)이 그대로 실려**. Claude는 이 설명"만" 보고, 실제 코드는 전혀 모른 채로 "이 질문엔 이 도구를 써야겠다"를 스스로 판단한다 — **이 판단 자체가 별도 로직이 아니라 Claude 모델 자체의 생성 결과**다(tool-use를 학습한 LLM이 다음 토큰으로 "일반 텍스트" 대신 "도구 호출 요청" 형태를 생성하는 것).
...
핵심은 `org.springframework.ai.model.tool.ToolCallingManager`(구현체 `DefaultToolCallingManager`)야:
...
이 왕복(1차 요청 → tool 실행 → 2차 요청 → 최종 답변)이 **Claude가 더 이상 tool을 요청하지 않을 때까지 자동으로 반복**되는 게 바로 지난번 말한 "단순 오케스트레이션"의 정체야 — `ChatModel.call()` 내부에서 다 처리되고, `ChatController` 입장에선 `.call().content()` 한 줄이 끝날 때까지 이 왕복이 몇 번 일어났는지조차 알 필요가 없어.
...
dstone-ai-engine.md 파일의 "7.1 Tool 호출 흐름"에 이 내용을 보강해줘

---

## 답변

Let's do a final consistency check (fences, TOC) and confirm.

---

## 답변

"7.1 Tool 호출 흐름" 절을 3단계(등록/통지/실행루프)로 나눠서 상세 다이어그램 2개(등록 흐름 flowchart, `ToolCallingManager` 시퀀스), JSON Schema 예시 표, 요약 표까지 보강했어. 겹치는 설명이 있던 7.2/7.3절은 이 새 내용을 가리키도록 정리해서 중복을 줄였고. 마크다운 코드펜스 짝도 정상 확인했어.

---

## 질문

ai의 자동 Tool호출결정 메카니즘을 어느정도까지 신뢰할 수 있을까?

---

## 답변

신뢰도는 "어느 정도까지"라는 질문에 걸맞게 **조건부**로 답해야 할 것 같아 — 무조건 믿어도 되는 것도, 못 믿을 것도 아니야. 실제로 어디까지 믿을 수 있고 어디서부터 위험한지 나눠볼게.

## 지난번 테스트가 증명한 것 / 증명하지 못한 것

우리가 검증한 건 **3개 질문 케이스**뿐이야:
- 지식베이스에 있는 내용 → 검색함 ✅
- 일반 수학 질문 → 검색 안 함 ✅
- 일반 프로그래밍 질문 → 검색 안 함 ✅

이건 "이 메커니즘이 존재하고 대체로 그럴듯하게 동작한다"는 증거는 되지만, **"신뢰할 수 있다"는 통계적 근거는 안 돼**. 3번 다 맞았다고 100번째도 맞는다는 보장이 없어 — LLM의 tool-call 판단은 규칙 기반이 아니라 **확률적으로 학습된 행동**이라서, 애매한 질문(일반 지식과 도메인 지식이 섞인 질문, 표현이 모호한 질문)에서는 같은 질문도 매번 다르게 판단할 수 있어(temperature/샘플링에 따른 비결정성).

## 신뢰도를 갉아먹는 구체적인 실패 유형

| 실패 유형 | 뭐가 문제인가 | 우리 시스템에서의 리스크 |
|---|---|---|
| **False negative** (호출해야 하는데 안 함) | LLM이 "알겠지"하고 자체 지식으로 답변 → 사실은 지식베이스 내용과 다를 수 있음(할루시네이션) | `searchKnowledgeBase` — 검색 없이 자기 지식으로 잘못 답할 수 있음 |
| **False positive** (안 해도 되는데 호출) | 불필요한 지연/비용(임베딩 API 호출) | 큰 피해는 아니지만 사용자 응답 시간 저하 |
| **설명 품질 의존** | `@Tool(description=...)`이 애매하면 판단 품질 자체가 떨어짐 | 지금은 설명을 꽤 명확히 써서 괜찮지만, 도구가 늘어나고 설명이 부실해지면 취약해짐 |
| **간접 프롬프트 인젝션** | 검색된 문서 내용 자체가 "이 지시를 무시하고 ~해줘" 같은 텍스트를 담고 있으면 이후 LLM 행동에 영향을 줄 수 있음 | 지금은 우리가 직접 올린 신뢰된 문서뿐이라 위험 낮음. 외부/사용자 업로드 문서를 받게 되면 얘기가 달라짐 |
| **다중 Tool 오선택** | Tool이 여러 개면 비슷한 것들 중 잘못 고를 가능성 ↑ | 지금은 2개뿐이라 낮음, Tool이 늘어날수록 커짐 |

## 결론 — 어디까지 믿어도 되나

**"참고용 정보를 가져오는 read-only Tool"** (지금 우리가 만든 두 개, `getCurrentDateTime`/`searchKnowledgeBase`)은 최악의 경우라도 "검색을 안 해서 좀 부정확하게 답한다" 정도라서 **위험도가 낮아 판단을 맡길 만해**.

반대로 **"실행하면 되돌릴 수 없는 side-effect가 있는 Tool"**(예: 이메일 발송, 데이터 삭제/수정, 결제 API 호출, 외부 시스템에 쓰기 작업)이라면 **이 자동 판단 메커니즘 하나에만 의존하면 안 돼** — false positive 한 번이 실제 사고로 이어지니까. 이런 건:
- 사람 확인(confirmation) 단계를 끼워 넣거나
- 호출 가능 여부를 서버 쪽에서 별도로 검증(allow-list, rate limit)하거나
- 아예 자동 호출 대상에서 빼고 명시적 API로만 노출

해야 해 — 이게 정확히 로드맵의 **Phase 4(governance)**가 다루기로 예정된 영역이야(Guardrail, rate limiting, 인증).

## 지금 당장 신뢰도를 실제로 높이려면

1. **`ragEnabled`(결정적) vs `toolsEnabled`(확률적) 용도 구분을 명확히**: "이 챗봇은 반드시 사내 규정을 참고해야 한다" 같은 요건이면 `toolsEnabled`에 판단을 맡기지 말고 `ragEnabled`로 매번 강제 검색시켜야 함(7.5절에 이미 적어둔 원칙)
2. **더 넓은 테스트셋으로 검증**: 지금 3개 질문 말고, 애매한 경계 케이스(일반 지식 + 도메인 지식이 섞인 질문) 여러 개로 실제 판단 정확도를 측정해봐야 진짜 신뢰도를 알 수 있어
3. **Observability 추가**(Phase 4 예정): Spring AI에 `ToolCallingObservationConvention`이 이미 있어서, 실제 운영에서 "언제 호출했고 언제 안 했는지" 로그로 남겨 사후 검증 가능하게 만드는 게 정석

원하면 지금 3개 케이스보다 훨씬 애매한 경계 질문들로 실제 테스트를 더 돌려서 판단 정확도를 좀 더 실증적으로 확인해볼까?
