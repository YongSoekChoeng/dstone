...
나는 dstone-common 에 kafka를 이용해서 사가+아웃박스패턴을 구현하는 구현체를 만들고 
dstone-boot, dstone-batch 등에서 해당 구현체를 간단한 형태로 불러다 쓸 수 있게 하고 싶은데 
어떤 설계모습이 좋을까 고민해줘.
...
좋은 고민입니다. 먼저 지금까지 확인한 이 프로젝트의 관례부터 짚고, 그 위에 설계를 얹는 게 맞다고 봅니다.

## 이 코드베이스가 이미 알려주는 제약

1. **DataSource/MyBatis/TransactionManager가 모듈마다 완전히 독립**입니다. `dataSourceCommon`이라는 이름은 boot/batch/batchadmin에서 각각 다른 물리 DB(`sampleDB`, `batchadmin` 등)를 가리키고, `ConfigMapper`/`ConfigTransaction`도 모듈별로 따로 정의되어 있습니다. → **아웃박스 테이블은 "공용 DB"가 아니라 각 모듈이 갖고 있는 자신의 DB에 있어야** 트랜잭션(비즈니스 insert + outbox insert가 한 커밋)이 성립합니다.
2. **`net.dstone.common.utils.KafkaUtil`이 이미 존재**하는데, Spring 빈이 아니라 순수 자바 싱글톤/팩토리 클래스입니다. 즉 common은 "Spring이 자동으로 주워가는 컴포넌트"가 아니라 "모듈이 명시적으로 가져다 조립하는 재료"를 제공하는 역할로 일관되어 있습니다(앞서 확인한 `@ComponentScan(basePackages={"net.dstone.boot"})` + `Config.java`의 명시적 `@Import` 패턴과 동일선상).
3. `dstone-batch`는 이미 Step/Tasklet 기반 주기 실행 구조를 갖고 있고, `dstone-boot`는 `@EnableAsync`가 켜져 있어 스케줄링 인프라를 얹기 쉽습니다.

이 세 가지를 보면, 자연스러운 결론은 **"common에는 엔진(로직)과 인터페이스, 스키마 템플릿만 두고, 실제 빈 배선(DataSource/SqlSession/KafkaTemplate 연결)은 각 모듈의 `Config.java`가 담당"** 하는 구조입니다. common이 자기 자신을 `@Configuration`으로 자동 등록하려 하면 지금 프로젝트 전체의 관례(명시적 `@Import` 화이트리스트)와 충돌합니다.

## 제안하는 모양

**아웃박스**: CDC(Debezium)는 별도 인프라(Kafka Connect 등)가 필요해서 지금 스택엔 과합니다 → **폴링 릴레이 방식**을 추천합니다.
- common: `OutboxMessage`(POJO), `OutboxDao`(인터페이스, MyBatis `SqlSessionTemplate`을 생성자로 받는 구현체), `OutboxRelay`(pending 조회 → KafkaTemplate 발행 → 상태 갱신, 순수 클래스, `@Component` 아님) + `schema/outbox.sql` 템플릿(배치 메타테이블처럼 수동 생성용).
- 모듈 쪽 "쓰는 코드"는 딱 한 줄:
  ```java
  @Transactional
  public void placeOrder(OrderVo order) {
      orderDao.insert(order);
      outboxService.append("order-events", order.getOrderId(), payload); // 같은 트랜잭션
  }
  ```
- 릴레이 실행 주체는 모듈마다 자연스러운 방식으로: **boot는 `@Scheduled`**로 `OutboxRelay.dispatchPending()` 호출, **batch는 `BaseJobConfig` 기반 Step/Tasklet**이 같은 메서드를 호출 — 이미 있는 배치 실행 주기에 얹으면 됩니다.

**사가**: 코레오그래피(각 서비스가 이벤트 듣고 알아서 다음 이벤트 발행)보다 **오케스트레이션(중앙 상태 저장 + 다음 스텝 지시)** 을 추천합니다. 이유는 이 프로젝트가 이미 `dstone-batchadmin`처럼 "중앙에서 상태를 보고 관리"하는 사고에 맞춰져 있고, 실패/보상 흐름을 한 곳(`SAGA_INSTANCE`/`SAGA_STEP_LOG` 테이블)에서 추적할 수 있어야 운영이 쉽기 때문입니다.
- common: `SagaDefinition`(스텝 목록), `SagaStepHandler<C,R>`(`handle`/`compensate`) 인터페이스, `SagaOrchestrator`(상태 테이블 읽고 다음 커맨드를 outbox에 적재하는 엔진, 역시 순수 클래스).
- 각 모듈은 자기 도메인의 `SagaStepHandler` 구현체를 자기 base package에 `@Component`로 등록(→ 지금 스캔 범위 안에 들어옴)하고, `@KafkaListener`로 응답 이벤트를 받는 구체 리스너도 모듈 쪽에 둡니다. common은 라우팅 로직만 제공.

## 핵심 트레이드오프

- 폴링 릴레이는 구현이 단순하지만 **발행 지연이 폴링 주기만큼** 생깁니다(수백ms~수초 정도 감내 가능한지 확인 필요). CDC로 가면 지연은 거의 0이지만 Debezium/Kafka Connect라는 인프라가 추가로 필요합니다.
- 오케스트레이션은 상태 추적이 쉬운 대신, `SagaOrchestrator`와 그 상태 테이블이 **단일 장애점/병목**이 될 수 있어 재시도·타임아웃 처리를 엔진 쪽에서 신경 써야 합니다.

이 방향(폴링 아웃박스 + 오케스트레이션 사가, common=엔진/인터페이스+모듈=배선)으로 괜찮으시면, 다음 단계로 실제 클래스 스켈레톤(패키지 구조, DDL, `Config.java` 배선 예시)까지 잡아드릴까요?

---

## 질문

이 설계대로 했을 때 패키지/클래스 구조,명이 어떻게 되는거야?

---

## 답변

좋습니다. 지금까지 확인한 이 프로젝트의 실제 관례(모듈별 독립 DataSource/SqlSessionTemplate, 문자열 namespace 기반 MyBatis, `Base*` 추상클래스는 common에 두고 실제 구현은 모듈에 둠, `Config*.java` + 명시적 `@Import` 배선)에 맞춰 이름까지 확정하면 아래와 같습니다.

## 1. dstone-common — 엔진 + 계약(인터페이스) + 스키마 템플릿만

```
net.dstone.common.messaging.outbox
├── OutboxMessage.java         // Vo: id, topic, msgKey, payload(JSON), status, retryCount, createdAt...
├── OutboxStatus.java          // enum: PENDING, SENT, FAILED
├── OutboxStore.java           // interface: insert(msg), findPending(limit), markSent(id), markFailed(id,err)
├── OutboxAppender.java        // interface: append(topic, key, payload)  ← 비즈니스 코드가 부르는 진입점
├── OutboxAppenderImpl.java    // OutboxStore만 감싸는 기본 구현체 (DB 종속 없음 → common에서 완결 가능)
└── OutboxRelay.java           // 엔진: dispatchPending(int batchSize) — OutboxStore로 조회, KafkaTemplate로 발행, 상태갱신

net.dstone.common.messaging.saga
├── SagaInstance.java          // Vo: sagaId, sagaType, status, currentStep, payload
├── SagaStatus.java            // enum: STARTED, STEP_DONE, COMPENSATING, COMPLETED, FAILED
├── SagaStepHandler.java       // interface<C,R>: handle(command), compensate(command)  ← 모듈이 구현
├── SagaStore.java             // interface: save/find/updateStatus  ← 모듈이 구현
└── SagaOrchestrator.java      // 엔진: start(sagaType,payload), onStepResult(...) — SagaStore + OutboxAppender 사용

src/main/resources/schema/messaging/
├── outbox_message.sql         // DDL 템플릿 (Spring Batch 메타테이블처럼 각 모듈 DB에 수동 실행)
├── saga_instance.sql
└── saga_step_history.sql

src/main/resources/sqlmap-template/messaging/
├── OutboxDao.xml.sample       // 실제 스캔 대상 아님. 모듈이 자기 sqlmap 폴더로 복사해서 커스터마이즈
└── SagaDao.xml.sample
```

`OutboxAppenderImpl`, `OutboxRelay`, `SagaOrchestrator`는 전부 **`@Component`/`@Configuration`이 아닌 순수 클래스**입니다 — `KafkaUtil`이 그런 것처럼, common은 "가져다 쓸 재료"만 제공하고 실제 빈 등록은 모듈이 합니다.

## 2. dstone-boot — 배선 + 실사용

```
net.dstone.boot.common.messaging.outbox
└── OutboxDao.java             // extends net.dstone.boot.common.biz.BaseDao, implements OutboxStore
                                //   sqlSessionCommon.insert("...OutboxDao.insert", vo) 식, AdminDao와 동일 패턴

net.dstone.boot.common.messaging.saga
└── SagaDao.java                // extends BaseDao, implements SagaStore (동일 패턴)

net.dstone.boot.common.config
└── ConfigMessaging.java        // @Component, @Bean: outboxStore, outboxAppender, outboxRelay, sagaStore, sagaOrchestrator
                                 //   → net.dstone.boot.common.config.Config 의 @Import 목록에 추가

net.dstone.boot.common.messaging.outbox
└── OutboxRelayScheduler.java   // @Component + @Scheduled(fixedDelay=...) → outboxRelay.dispatchPending(100)

src/main/resources/sqlmap/common/messaging/
├── OutboxDao.xml               // namespace="net.dstone.boot.common.messaging.outbox.OutboxDao"
└── SagaDao.xml                 // namespace="net.dstone.boot.common.messaging.saga.SagaDao"
```

**실사용 예** (기존 `sample/kakao` 스타일과 동일한 자리에):
```
net.dstone.boot.sample.order.service.OrderService     // OutboxAppender 주입받아 사용 (기존 KafkaService.publish 자리를 대체)
net.dstone.boot.sample.order.saga.OrderSagaStepHandler // @Component, implements SagaStepHandler<...>
net.dstone.boot.sample.order.saga.OrderSagaListener    // @KafkaListener(topics="order-saga-reply", groupId=...) → sagaOrchestrator.onStepResult(...) 호출
```

## 3. dstone-batch — 배선 + 배치 릴레이(Job)

batch는 자체 `BaseDao`가 없으므로(리더/라이터/태스크릿이 `SqlSessionTemplate`을 직접 씀) Dao는 최소 형태로:

```
net.dstone.batch.common.messaging.outbox
└── OutboxDao.java              // implements OutboxStore, @Autowired @Qualifier("sqlSessionCommon") SqlSessionTemplate 직접 사용

net.dstone.batch.common.config
└── ConfigMessaging.java        // outboxStore/outboxRelay 빈 정의 → batch의 Config.java @Import에 추가

net.dstone.batch.sample.jobs.jobOutboxRelay
├── OutboxRelayJobConfig.java   // extends BaseJobConfig, @AutoRegJob(name="outboxRelayJob")
└── items/OutboxRelayTasklet.java  // Tasklet: outboxRelay.dispatchPending(500) 호출
...
| 계층 | 위치 | 성격 |
|---|---|---|
| 모델/계약 | `net.dstone.common.messaging.*` (인터페이스, Vo, enum) | 모듈 무관, 순수 |
| 엔진 | `net.dstone.common.messaging.*` (`OutboxRelay`, `SagaOrchestrator`) | Spring/DB 비종속 로직, 생성자로 인터페이스만 받음 |
| DB 접근 구현체 | `net.dstone.{boot,batch}.common.messaging.*` (`OutboxDao`, `SagaDao`) | 모듈별 실제 SqlSessionTemplate/네임스페이스 |
| 빈 배선 | `net.dstone.{boot,batch}.common.config.ConfigMessaging` | 각 모듈 `Config.java`의 `@Import`에 한 줄 추가 |
| 트리거 | boot=`@Scheduled`, batch=`@AutoRegJob` | 모듈의 기존 실행 방식 그대로 재사용 |
| 실제 사용 | `net.dstone.{boot,batch}.sample.{feature}.*` | `OutboxAppender`/`SagaStepHandler` 주입받아 씀 |

이 구조로 가면 비즈니스 코드가 다루는 건 `OutboxAppender.append(...)`와 `SagaStepHandler` 구현 정도뿐이고, 나머지(테이블 스키마, 폴링 로직, 상태 전이)는 common의 엔진 코드 재사용입니다. 다음으로 넘어가면 `outbox_message.sql` DDL과 `OutboxStore`/`OutboxRelay` 실제 코드부터 만들어드릴까요?
