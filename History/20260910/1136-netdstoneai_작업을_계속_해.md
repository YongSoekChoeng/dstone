...
dstone-ai-engine 의 주석이 보다 친절했으면 좋겠어. 한글 사용자 입장에서 조금 어색한 표현도 있고... "{@link"로 시작되는 이런 표현들도 굳이 이렇게 표현하지 않고 그냥 표현되어도 될 것 같아.

---

## 답변

Now `PiiGuardrailProperties.java`, `PiiPatterns.java`, `PiiScanResult.java`.

---

## 답변

All `{@link}` usages removed. Now let's compile to make sure nothing broke.

---

## 답변

Found three genuine leftover spacing awkwardness. Let's fix them.

---

## 답변

Now let's do the final compile check and a broader sweep for other stale/awkward remnants.

---

## 답변

35개 파일의 주석을 정리했고, 컴파일도 정상입니다.

## 완료

**`{@link ...}` 전부 제거** (18곳, 12개 파일) — 이제는 그냥 클래스/메서드 이름을 평문으로 풀어 씁니다. `{@code ...}`도 정리 과정에서 자연스럽게 다 사라졌습니다.

**어색한 한국어 표현 정리**
- "RAG 는", "ConfigTool 이", "chatClient 의" 같이 영문 식별자 뒤에 조사가 띄어써진 곳들을 붙여 썼습니다.
- ChatController의 `// 세션ID 가 진행한 대화 누적치 가 적용된 요청스펙` 같은 압축된 명사구 나열을 "세션 ID를 걸어서 지금까지의 대화 히스토리가 이어지도록 한 요청 스펙"처럼 실제 문장으로 풀어 썼습니다.
- 여러 절을 대시(-)와 괄호로 욱여넣은 문장들을 짧은 문장 여러 개로 나눴습니다 (`ApiKeyAuthFilter`, `ConfigChatClient`, `RateLimiter` 등).
- 서로 모순돼 보이던 주석 하나를 고쳤습니다 — `ChatController`의 "강제옵션(Anthropic은 지원하지 않음)"이 실제로는 "Anthropic 전용 기능"이라는 뜻이었는데, 반대로 읽히는 문구였어서 정정했습니다.
- 자바독 안에 markdown bold(`**LLM이 스스로 판단**`)처럼 렌더링 안 되는 문법이 섞여 있던 곳(`RetrievalTools`)도 정리했습니다.
- `AiTool.java`에 남아있던 옛 `agent` 패키지 언급(현재는 `tools.rag`/`tools.sample`로 이름이 바뀐 상태)도 최신 구조에 맞게 고쳤습니다.
- `ChatController`에 없던 클래스 레벨 설명을 하나 추가해서, 처음 보는 사람이 이 컨트롤러가 뭘 하는지 바로 감을 잡을 수 있게 했습니다.

`ConfigAspect.java`는 dstone-boot/batch/batchadmin과 동일한 공용 패턴이라 일부러 손대지 않았습니다 — 여기만 표현을 바꾸면 오히려 다른 모듈들과 스타일이 어긋납니다.
...
Now let's fix the gateway/provider line, the rag/retrieval tree (remove misplaced RetrievalTools), and the stale `agent/` entry — replacing it with the real `tools/` package.

---

## 답변

Now fix the stale `agent.tool` reference in section 14.

---

## 답변

Let's add a unifying "big picture" diagram for Phase 4 that ties all four governance/observability layers together, since currently each is only shown in isolation.

---

## 답변

Now let's update the two index docs that summarize ai-engine's Phase 4 status.
...
`/app/dstone/docs`를 훑어서 지금까지의 작업(Phase 4 전 기능 + 패키지 구조 변경)과 관련 있는 문서만 정확히 갱신했습니다. 새로 만들 문서는 없었습니다 — 전부 기존 `09.dstone-ai-engine.md`(주 문서)와 그걸 요약해서 링크하는 인덱스 2곳에 이미 자리가 있었습니다.

**`09.dstone-ai-engine.md`**
- **오래된 패키지 경로 정리**: 실제 코드는 `agent/tool/sample/` → `tools/sample/`, `rag/retrieval/RetrievalTools.java` → `tools/rag/RetrievalTools.java`로 이미 바뀌어 있었는데 문서의 패키지 구조 트리(3절)와 7.5절 본문이 옛 경로를 그대로 갖고 있어서 실제 코드와 정확히 일치하도록 고쳤습니다. 삭제된 `gateway/provider/` 빈 디렉터리도 트리에서 뺐습니다.
- **비주얼 보강**: 8절 도입부에 새 다이어그램("Phase 4 한눈에 보기 — 요청 하나가 통과하는 4개의 관문")을 추가했습니다. 지금까지는 인증/rate limit/PII/사용량 로깅이 각자 절에 따로 그려져 있어서 전체 그림을 머리속으로 조립해야 했는데, 하나의 flowchart로 Servlet Filter 2개 + ChatClient Advisor 2개가 실제로 어떤 순서로 요청을 통과시키는지, 각각 실패하면 어떤 응답이 나오는지를 색깔로 구분해 한눈에 보이게 했습니다.
- 로드맵/상태 표는 이미 이전 턴에서 갱신돼 있어 그대로 두었습니다.

**`01.index.md`, `README.md`**
- dstone-ai-engine 한 줄 요약이 "Governance·API Key 인증(Phase 4, 진행 중)"으로만 돼 있던 걸 "Governance(API Key 인증·Rate Limit·PII Guardrail) & Observability(사용량 로깅)(Phase 4, 진행 중)"로 갱신해서, 인덱스만 봐도 Phase 4에서 뭐가 끝났는지 알 수 있게 했습니다.

`04.cloud-architecture.md`/`03.build.md`/`02.environment.md`/`software/*.md`는 확인해봤지만 이번 작업(rate limit·PII·사용량 로깅, 코드 주석 정리)이 배포 방식이나 인프라 요구사항 자체를 바꾸진 않아서 손댈 내용이 없었습니다 — Redis를 새로 하나 더 쓰는 게 아니라 기존 세션용 Redis를 그대로 재사용하는 식이라 인프라 표는 그대로 유효합니다.
