...
<pasted_content id="052a">
아래의 yml 에서...
sqlList: list<string> 에 값을 넘기는 방법은 무엇인가?(샘플 필요)
그리고 list안에 단순 string이 아닌 객체가 될 수도 있을 텐데... 이 경우 어떻게 표현되고 step에서는 어떻게 받나? 샘플을 보여줘.

workflow:
  id: sample-foreach-parallel
  description: forEach 동작 확인용 테스트 Workflow - TOOL step 하나를 sqlList 항목 수만큼 동시 실행한다.
  maxIterations: 5
  inputs:
    sqlList: list<string>
  steps:
...
객체 리스트 경우를 실제 코드로 확인해 보겠습니다(타입 검사 + `{{item.키}}` 채우기).

---

## 답변

## 1. `sqlList: list<string>`에 값을 넘기는 방법

실행 요청 JSON의 **`variables`** 안에 `inputs`에 선언한 이름(`sqlList`)과 같은 키로 넣습니다. `message`는 항상 필수라서 이 Workflow가 쓰지 않더라도 아무 문자열이나 넣어야 합니다.
...
curl -X POST http://localhost:8081/api/ai/workflow/sample-foreach-parallel/execute \
  ...
  -d '{
        "message": "검사",
        "variables": {
          "sqlList": ["SELECT 1 FROM dual", "SELECT name FROM dept", "SELEC 1 FROM dual"]
        }
      }'
```
- 비동기로 돌리려면 `/execute` 대신 `/submit`을 쓰고, 돌려받은 `executionId`로 `/status/{executionId}`를 조회합니다.
- dstone-boot "Workflow 테스트" 화면에서는 workflowId에 `sample-foreach-parallel`, message에 아무 글자, variables 칸에 `{"sqlList": ["SELECT 1 FROM dual", "SELEC 1 FROM dual"]}`를 넣으면 됩니다.

**값이 흘러가는 길**
```
variables.sqlList = ["SELECT 1 FROM dual", "SELECT name FROM dept", "SELEC 1 FROM dual"]
   │ ① 실행 전 타입 검사: list<string>인가? 아니면 400
   ▼
context.inputs.sqlList                     ← YAML inputs.sqlList와 같은 이름
   │ ② forEach: inputs.sqlList → 항목 3개 → 동시에 3번 실행
   ├─ [0] item = "SELECT 1 FROM dual"    → input {sql: "{{item}}"} → validateSqlSyntax(sql="SELECT 1 FROM dual")
   ├─ [1] item = "SELECT name FROM dept" → validateSqlSyntax(sql="SELECT name FROM dept")
   └─ [2] item = "SELEC 1 FROM dual"     → validateSqlSyntax(...) → 문법 오류 → 이 반복 실패
   ▼
steps.validate-each.items = [ {input, output, text, error}, ×3 ]  → 하나라도 실패했으므로 onFailure: FAIL
```

**거절되는 요청 (HTTP 400, 실행 전)**

| `variables` | 결과 |
|---|---|
| `{}` (sqlList 없음) | `[sqlList(없음)]` |
| `{"sqlList": "SELECT 1"}` (리스트 아님) | `[sqlList(list<string> 타입이어야 함)]` |
| `{"sqlList": ["SELECT 1", 123]}` (항목 하나가 숫자) | 위와 같음 |
| `{"sqlList": []}` | 통과. 0번 실행하고 SUCCESS |

## 2. 리스트 항목이 객체일 때

타입을 **`list<object>`**로 선언하고, step에서는 **`{{item.키}}`**로 꺼냅니다. `itemVariable`로 `item` 대신 읽기 쉬운 이름을 붙일 수도 있습니다.

**YAML 샘플** (설명용 예시로, 실제 파일은 없습니다)
```yaml
workflow:
  id: sample-foreach-objects
  description: SQL 객체 목록을 항목마다 문법 검사한 뒤, 이름을 붙여 검토 의견을 받는다
  maxIterations: 5
  inputs:
    queries: list<object>                 # 항목 하나 = {name, sql, tables}
  steps:

    - id: validate-each                   # ❶ TOOL: 객체에서 필요한 키만 골라 인자로
      type: TOOL
      ref: validateSqlSyntax
      forEach: inputs.queries
      itemVariable: q                     # {{item}} 대신 {{q}}
      input:
        sql: "{{q.sql}}"                  # 문자열 키 하나만 꺼냄
      onFailure: FAIL

    - id: review-each                     # ❷ AGENT: 객체의 여러 키를 글자 안에 섞어서
      type: AGENT
      ref: <agent-id>
      forEach: inputs.queries
      itemVariable: q
...
```json
{
  ...
  "variables": {
    "queries": [
      { "name": "emp-top10", "sql": "SELECT * FROM emp WHERE ROWNUM <= 10", "tables": ["EMP"] },
      { "name": "dept-all",  "sql": "SELECT * FROM dept",                    "tables": ["DEPT"] }
    ]
  }
}
```

**step이 받는 모양** (아래 값은 실제로 코드를 돌려서 확인한 것입니다)
```
반복 [0]: q = {name: "emp-top10", sql: "SELECT * FROM emp WHERE ROWNUM <= 10", tables: ["EMP"]}

TOOL input 템플릿                          →  채운 결과(Tool 인자)
  sql:    "{{q.sql}}"                      →  "SELECT * FROM emp WHERE ROWNUM <= 10"   (문자열)
  tables: "{{q.tables}}"                   →  ["EMP"]                                 (리스트 그대로)
  label:  "[{{q.name}}] {{q.sql}}"         →  "[emp-top10] SELECT * FROM emp ..."     (섞으면 글자)

AGENT input 템플릿                         →  채운 결과(LLM user 메시지)
  대상: {{q.name}}                          →  대상: emp-top10
  원본: {{q}}                               →  원본: {"name":"emp-top10","sql":"SELECT ...","tables":["EMP"]}  (객체 통째로면 JSON 글자)
```

**꺼내는 규칙**
- **`{{q.키}}`** — 객체 안의 값 하나를 꺼냅니다. 더 깊은 값도 꺼낼 수 있습니다(`{{q.tables.0}}` → `"EMP"`).
- **TOOL 인자에서 값 전체가 `"{{...}}"` 하나뿐일 때** — 원래 타입이 그대로 넘어갑니다(리스트는 리스트, 숫자는 숫자).
- **AGENT `input`이나 다른 글자와 섞일 때** — 항상 글자가 됩니다. 객체나 리스트는 JSON 글자로 들어갑니다.

**알아둘 점**
- **안쪽 키는 요청 때 검사되지 않습니다.** `list<object>`는 각 항목이 객체(맵)인지만 봅니다. `name`/`sql` 키가 있는지는 확인하지 않습니다.
  - 키가 빠진 항목이 오면 그 반복이 `{{q.sql}}: 값을 찾을 수 없습니다`로 실패하고, 그래서 step 전체가 실패합니다.
  - 빠질 수 있는 키는 `{{q.owner ?? inputs.message}}`처럼 `??`로 대체값을 줍니다.
- **안쪽 키 이름은 기동 때도 검사되지 않습니다.** 엔진은 `q`로 시작하는지만 보고, `q.sqll` 같은 오타는 실행해 봐야 드러납니다.
- **항목 모양이 섞이면 거절됩니다.** 요청에 객체와 문자열을 섞어 보내면(`[{...}, "x"]`) 400입니다.
- **객체 안 리스트를 또 반복할 수는 없습니다.** forEach는 한 단계만 됩니다. 필요하면 앞 step의 `output`으로 리스트를 만들어 다음 step에서 `forEach: steps.<id>.output.<키>`로 반복합니다.

위 객체 리스트 예시를 `workflows/sample/`에 실제 샘플 파일로 추가하고 가이드 2.5.5 패턴 D와 연결할까요?

---

## 질문

아래의 표현에서 사용된 {{item}} 의 정확한 의미를 모르겠어. 왜 중괄호가 두겹으로 사용된건지?
[0] item = "SELECT 1 FROM dual"    → input {sql: "{{item}}"} → validateSqlSyntax(sql="SELECT 1 FROM dual")
...
`{{item}}`은 **"이 자리에 `item`이라는 이름의 값을 넣어라"는 빈칸 표시**입니다. 중괄호 두 겹 `{{ }}`는 "여기는 글자가 아니라 채워 넣을 자리"라는 표시이고, 안쪽의 `item`이 무엇을 넣을지를 가리키는 이름입니다.
...
엔진은 `forEach` 리스트의 항목마다 이 step을 한 번씩 실행합니다. 실행할 때마다 **이번 항목을 `item`이라는 이름으로 잠깐 넣어 두고** 빈칸을 채웁니다.
...
Java의 for-each 문과 같은 구조입니다.

```java
for (String item : inputs.sqlList) {      // forEach: inputs.sqlList
    validateSqlSyntax(item);              // sql: "{{item}}"
}
```

- `item`은 **반복 변수 이름**입니다. 기본값이 `item`이고, `itemVariable: sql`처럼 바꾸면 `{{sql}}`로 씁니다.
- `item`은 그 step의 반복 안에서만 존재합니다. 다른 step에서 `{{item}}`을 쓰면 기동이 실패합니다.
...
| `{role}` | Agent `prompt`의 **변수 자리** | Spring AI `PromptTemplate` |
| `${APP_HOME}` | **환경 설정값** 자리(엔진이 켜질 때 한 번 치환) | `YamlDefinitionLoader` |
| **`{{item}}`** | Workflow **실행 중 값** 자리(step마다 치환) | `common.template.Template` |

예를 들어 한 겹으로 `sql: {item}`이라고 적으면, YAML은 이것을 "`item`이라는 키를 가진 맵"으로 읽어 버립니다. 두 겹 `{{ }}`는 다른 표기들과 겹치지 않으므로 엔진이 "여기가 채울 자리"라고 확실히 알 수 있습니다. Mustache, Jinja, Handlebars 같은 템플릿 도구가 쓰는 관례이기도 합니다.
...
```yaml
sql: "{{item}}"     # ✅ 글자 "{{item}}" → 실행 때 엔진이 채움
sql: {{item}}       # ❌ YAML이 맵 안의 맵으로 읽으려다 기동 실패
```

### 4. 같은 규칙의 다른 빈칸들

`{{ }}` 안에는 항상 **실행 컨텍스트에서 값을 찾아갈 경로**가 들어갑니다. `item`은 그중 하나입니다.
