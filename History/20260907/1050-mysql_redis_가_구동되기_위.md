...
이거 정상인가? jysn007@DESKTOP-DVQ9FB6:~$ kubectl delete -f dstone-boot/k8s/
error: the path "dstone-boot/k8s/" does not exist
...
정상입니다 — 버그가 아니라 그냥 위치 문제예요. 프롬프트가 `jysn007@DESKTOP-DVQ9FB6:~$` 즉 홈 디렉터리(`/home/jysn007`)인데, `dstone-boot/k8s/`는 상대 경로라서 `/home/jysn007/dstone-boot/k8s/`를 찾다가 없으니 저 에러가 난 겁니다. 리포지토리는 `/app/dstone`에 있으니 거기서 실행해야 합니다.
...
```bash
kubectl delete -f /app/dstone/dstone-boot/k8s/
```
...
dstone/docs/software/rabbitmq.md 문서에 
[7. dstone 프로젝트에서의 역할]을 [8. dstone 프로젝트에서의 역할]로 바꿔주고
[7. 기본 설정 Import] 라는 항목을 넣어주고 아래의 파일을 임포트하는 내용을 추가해줘.
dstone/docs/data/dstone-batch-postman-collection.json

---

## 답변

`docs/software/rabbitmq.md` 수정 완료했습니다. 목차와 본문 모두 반영했습니다.

- 기존 `[7. dstone 프로젝트에서의 역할]` → `[8. dstone 프로젝트에서의 역할]`로 번호 변경
- 새 `## 7. 기본 설정 Import` 섹션 추가: `docs/data/dstone-batch-postman-collection.json`(컬렉션 이름 "배치샘플", `common`/`job001`~`job006` 폴더, `dstone-batch` REST API `localhost:6081` 호출용)를 Postman에 Import하는 절차 안내

참고로 이 파일은 실제로는 RabbitMQ가 아니라 `dstone-batch` REST API 테스트용 컬렉션인데, 문서 내용에는 그대로 반영해뒀습니다 — 혹시 원래 넣으려던 문서가 `rabbitmq.md`가 아니라 `dstone-batch.md`였다면 말씀해주세요.
...
dstone/docs/software/rabbitmq.md 문서에 
[7. 기본 설정 Import] 라는 항목을 넣어주고 아래의 파일을 임포트하는 내용으로 수정해줘.
dstone/docs/data/rabbitmq-basic-config.json

---

## 답변

`docs/software/rabbitmq.md`의 7절 내용을 올바른 파일 기준으로 수정했습니다.

`docs/data/rabbitmq-basic-config.json`은 postman 컬렉션이 아니라 RabbitMQ 표준 **Definitions(정의) export 파일**이었습니다 — vhost(`/dstone-mq`, `/`), 사용자(`guest`, `jysn007`), 큐/익스체인지/바인딩까지 6절에서 수동으로 만드는 구성이 그대로 들어있더군요. 그래서 내용을 이렇게 바꿨습니다:

- **방법 A**: 관리 콘솔(`http://localhost:15672`) → Admin → Overview → Definitions에서 파일 업로드
- **방법 B**: `sudo rabbitmqctl import_definitions docs/data/rabbitmq-basic-config.json`
- 비밀번호는 해시로만 들어있어 평문 복구가 안 되니, import 후 `change_password`로 재설정하고 `application.yml`의 `ENC(...)` 값도 맞춰 다시 암호화해야 한다는 점도 덧붙였습니다.
