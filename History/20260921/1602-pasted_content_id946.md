...
agents yml 에서 description 를 활용하나 ?

---

## 답변

아니요, 지금은 안 씁니다. `AgentDefinition.description`은 레코드 필드로만 존재하고, 코드 어디에서도 `.description()`을 호출해서 읽는 곳이 없습니다 — `AgentRegistry.resolve()`도, `ChatController`/`AgentStepRunner`도 이름(`name`)과 프롬프트(`prompt`)만 쓰고 description은 그냥 지나칩니다.

참고로 `WorkFlowDefinition.description`은 반대로 실제로 쓰입니다 — `GET /api/ai/workflow`가 등록된 Workflow 목록을 `id`+`description`으로 응답하기 때문입니다(`WorkFlowController`/`WorkFlowSummary`). Agent 쪽은 그런 목록 조회 API 자체가 없어서, `description`은 순전히 yml 파일을 사람이 읽을 때 참고하는 문서화용 필드입니다.
