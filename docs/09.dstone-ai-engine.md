...
| 4 | 🚧 진행 중 (`governance.auth`는 완료, 실동작 검증됨) | `governance.auth` — API Key 인증(`ApiKeyAuthFilter`/`CallerContext`). 남은 것: rate limit/비용 트래킹/PII 필터/Guardrail(`governance`), 토큰·비용·트레이싱·Eval(`observability`) |
