...
**아직 실제 인프라(Redis/PostgreSQL+pgvector/Ollama/Anthropic API/실제 MCP 서버)에 대고 라이브
호출로 검증하지 못한 부분**(이 문서를 갱신한 세션에서는 컴파일/패키징 검증까지만 진행했다 - 다음
배포/테스트 세션에서 아래를 확인할 것):

- `/execute`가 APPROVAL 스텝에서 실제로 `WAITING_APPROVAL`을 반환하고, `/decision` 호출 후 실제로
  재개되는지
- `AI_WORKFLOW_EXECUTION`/`AI_WORKFLOW_EXECUTION_STEP_HISTORY`에 실제로 스텝마다 영속화되는지,
  `parallelGroup`이 그룹 단위로 올바르게 기록되는지
- `execution.log`의 `WF_STEP` 로그 라인이 실제 실행 순서/내용과 일치하는지
- `RagRetrievalChain`(RetrievalAugmentationAdvisor 기반)이 기존과 동일하게 검색-증강하는지,
  `allowEmptyContext(true)`가 의도대로 "컨텍스트 없어도 거부하지 않음"으로 동작하는지
- 실제 MCP 서버(예: `@modelcontextprotocol/server-filesystem`) 접속과 Tool 등록/호출
- dstone-boot의 신규 관리자 화면(`/ai/admin/workflow/*`, `/ai/admin/document/*`)이 실제 엔진과
  end-to-end로 맞물리는지

2026-09-15 최초 재설계 당시의 라이브 검증 기록(일반 채팅/Tool 호출/`oracle-to-postgresql` 동기
실행/기존 비동기 submit+status)은 이번 개편으로 API 계약이 바뀐 부분(특히 실행 모델)이 있어 그대로
유효하다고 보긴 어렵다 - 재검증이 필요하다.
