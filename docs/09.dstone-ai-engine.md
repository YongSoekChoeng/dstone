...
│   ├── CapabilityDefinition.java    # 설정 한 줄(name/promptName/toolsEnabled/ragEnabled)을 담는 레코드
│   └── CapabilityRegistry.java      # dstone.ai.capability.definitions 바인딩 + 이름으로 조회
...
| 4 | 🚧 진행 중 (API Key 인증/rate limit/PII Guardrail/사용량 로깅 완료, 전부 실동작 검증됨) | API Key 인증(`common.filter.ApiKeyAuthFilter`). caller별 요청 제한(`common.filter.RateLimitFilter`). PII 탐지/마스킹(`governance.guardrail`). 토큰 사용량/비용/지연시간 로깅(`governance.usage`). 남은 것: sensitive-word 등 다른 Guardrail, Eval 결과 로깅 |
...
| `capability` | String | 지정 시 `CapabilityRegistry`에 등록된 이름으로 `promptName`/`toolsEnabled`/`ragEnabled` 기본값을 채운다(7.7절). 위 세 필드를 요청에 직접 넣으면 그 값이 항상 우선한다. 등록 안 된 이름이면 에러 |
...
| Phase | 설정 키 영역 | 실제 패키지 | 상태 | 계획 |
|---|---|---|:---:|---|
| 4 | `governance.auth` | `common.filter`/`common.context` | ✅ 완료 (실동작 검증됨, 8.2절 참고) | API Key 인증 — `ApiKeyAuthFilter`/`CallerContext`(전용 `ApiKeyProperties`는 2026-09-14 삭제, `ConfigProperty.getListProperty()`로 대체) |
| 4 | `governance.ratelimit` | `common.filter` | ✅ 완료 (실동작 검증됨, 8.3절 참고) | caller별 요청 제한 — `RateLimitFilter`(Redis 고정 윈도우 카운터도 이 클래스 안에 직접 구현. 전용 `RateLimitProperties`/`RateLimiter`는 2026-09-14 삭제) |
| 4 | `governance.guardrail` | `governance.guardrail` | ✅ 완료 (실동작 검증됨, 8.4절 참고) | PII 탐지/마스킹 — `PiiGuardrailAdvisor`/`PiiGuardrailProperties`/`PiiPatterns`(ChatClient CallAdvisor) |
| 4 | `observability.usage` | `governance.usage` | ✅ 완료 (실동작 검증됨, 8.5절 참고) | 토큰 사용량/비용/지연시간 로깅 — `UsageLoggingAdvisor`/`UsageProperties`(ChatClient CallAdvisor). "비용 트래킹"은 이 기능으로 갈음(8.5절 참고) |
| 4 | `governance` (다른 종류의 Guardrail) | `governance.guardrail` | ⏳ 예정 | sensitive-word 차단 등 — Spring AI 기본 제공 `SafeGuardAdvisor`를 `governance.guardrail`에 이어서 붙이는 방식을 검토 |
| 4 | `observability` (Eval) | (미정) | ⏳ 예정 | Eval 결과 로깅 — 평가 데이터셋/채점 로직 등 이 모듈에 없는 전제가 먼저 필요 |
