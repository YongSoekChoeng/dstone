...
StepOutcome 팩토리 메서드(runtime.step.StepOutcome):
  ...
  successWithData(text, data)  → Success(text, data)                    (structuredOutput=true AGENT/TOOL)
...
sequenceDiagram
    ...
    CB-->>TE: 원본 응답(로컬 Tool은 Spring AI가 JSON 문자열로 한 번 감쌈,\nMCP Tool은 content 배열이 JSON으로 직렬화됨)
    TE->>TE: unwrap() — 로컬 Tool의 JSON 문자열 감싸기, MCP의 content 배열\n둘 다 풀어서 사람이 읽는 순수 텍스트로 맞춤(2026-09-23 추가)
    ...
    alt 성공 & structuredOutput=false(기본값)
        TSR-->>WE: StepOutcome.success(normalized)  ← Tool의 응답 문구가 아니라 "검증받은 원본 값"을 그대로 전달
    else 성공 & structuredOutput=true(2026-09-23 추가)
        TSR->>TSR: toolResult를 ToolPayload{primaryText,data}로 파싱 시도
        alt 파싱 성공
            TSR-->>WE: StepOutcome.successWithData(payload.primaryText(), payload.data())
        else 파싱 실패(대부분의 Tool, MCP Tool 전부 포함)
            TSR-->>WE: StepOutcome.success(toolResult)  ← Tool이 실제로 돌려준 응답을 그대로 전달(실패 아님)
        end
    else 실패
...
- **OUT**: `StepDefinition.structuredOutput`에 따라 둘로 갈린다(2026-09-23부터 - 그 전까지는 TOOL이
  이 필드를 아예 읽지 않고 항상 아래 "기본값" 쪽으로만 동작했다).
  - **`false`/미설정(기본값)** — 성공하면 Tool의 응답 문구가 아니라 **검증받은 원본 값**
    (`normalized`)이 다음 step으로 이어진다("통과했습니다" 같은 문구 자체는 다음 step에 새 정보가
    아니기 때문). `SqlSyntaxTool`처럼 "이 값이 맞는가"만 확인하는 검증형 Tool을 염두에 둔 값이다.
  - **`true`** — 성공하면 반대로 **Tool이 실제로 돌려준 응답**이 다음 step으로 이어진다 -
    `list_directory`/`read_text_file`처럼 "새 데이터를 가져오는" Tool을 염두에 둔 값이다. Tool이
    `runtime.tool.ToolPayload(primaryText, data)` 모양으로 응답했으면 그 `data`까지 살려서
    `{stepId.키}`로 참조 가능하게 넘기고, 그 모양이 아니면(MCP Tool은 전부 여기 해당 - MCP는
    구조화된 `structuredContent`를 아예 안 주고 텍스트 `content`만 준다) 응답 텍스트 전체를
    `primaryText`로 쓴다. **AGENT의 `structuredOutput:true`와 달리, 이 모양을 지키지 않아도 실패로
    보지 않는다** - 애초에 대부분의 Tool은 이 스키마를 알지도 못하므로, "형식이 다르다"는 게
    비정상 신호가 아니라 정상적인 다수 사례이기 때문이다(graceful fallback - AGENT는 정반대인
    fail-closed).
  - 실패는 `structuredOutput` 값과 무관하게 동일하다 - 원본 값 뒤에 `[검증 결과] <사유>`가 덧붙는다
    (`onFailure`로 되돌아간 step이 "무엇을, 왜 고쳐야 하는지" 둘 다 볼 수 있도록).
...
- **MCP 클라이언트** — `common.config.ConfigMcp`가 `resources/mcp/*.yml`(`McpServerDefinition`,
  ...
  sample-mcp-filesystem-list/execute`). TOOL step 쪽은 `read-notes` step에
  `structuredOutput: true`를 줘서, `read_text_file`이 실제로 읽어온 파일 내용을 원본 입력값 대신
  Workflow의 최종 `resultText`로 그대로 돌려받는다([6.3절 ⑤ TOOL](#6-step-inout-완전-참조--workflow--step--agentapprovaltool--llm)
  참고, 2026-09-23 추가) - 이 기능이 없었을 때는 실행 이력의 `success: true`로 성공 여부만 확인할
  수 있었고 실제 파일 내용은 다음 step에 전달할 방법이 없었다.
  ...
  **MCP Tool의 원본 응답은 로컬 Tool과 JSON 모양이 다르다.** `SyncMcpToolCallback`(spring-ai-mcp
  ...
  문자열 감싸기, MCP의 content 배열) 둘 다 풀어서 사람이 읽는 순수 텍스트로 맞춰 준다 - 이 수정이
  없으면 `structuredOutput: true`를 켜도 다음 step이 받는 값이 저 JSON 배열 텍스트 그대로 남는다.
...
**TOOL step에도 `structuredOutput: true`를 쓸 수 있다(2026-09-23부터)** - 다만 의미가 AGENT와
정반대다. TOOL의 기본값(`false`/미설정)은 성공해도 Tool의 응답이 아니라 **검증받은 원본 입력을
그대로** 다음 step에 넘긴다(`SqlSyntaxTool`처럼 "이 값이 맞는가"만 확인하는 검증형 Tool을 염두에 둔
값). `structuredOutput: true`를 주면 반대로 **Tool이 실제로 돌려준 응답**을 다음 step에 넘긴다 -
"새 데이터를 가져오는" Tool(MCP Tool 전부 포함)에 쓴다:

```yaml
    - id: read-file
      type: TOOL
      ref: read_text_file                          # 예: mcp/*.yml로 등록한 MCP filesystem 서버의 Tool
      inputTemplate: '{"path":"/data/report.txt"}'
      structuredOutput: true      # 원본 입력(경로 문자열) 대신, read_text_file이 실제로 읽어온 파일 내용을 돌려받음
      onSuccess: SUCCESS
      onFailure: FAIL
```

Tool이 `runtime.tool.ToolPayload(primaryText, data)` 모양의 JSON으로 응답하면 `data`까지 살아서
`{stepId.키}`로 참조할 수 있지만, 그 모양이 아니어도(대부분의 Tool·모든 MCP Tool) 실패로 보지
않고 응답 텍스트 전체를 그대로 넘긴다 - AGENT의 `structuredOutput: true`는 스키마를 안 지키면
실패로 처리하는 것과 반대다(자세한 이유는 [6.3절 ⑤ TOOL](#6-step-inout-완전-참조--workflow--step--agentapprovaltool--llm) 참고).
...
- MCP Tool이 `runtime.tool.ToolPayload` 모양(`{"primaryText":...,"data":{...}}`)으로 직접 응답하는
  경로 - 1절에서 확인했듯 `structuredContent`가 Spring AI에 노출되지 않으므로 MCP Tool로는 이
  경로를 재현할 수 없었다. 로컬 `@AiTool`이 `ToolPayload`를 직접 반환하는 경우로 별도 검증이 필요하다
