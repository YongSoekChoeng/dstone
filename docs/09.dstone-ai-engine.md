...
| 3 | ✅ 완료 (`toolsEnabled`는 실동작 검증됨, `requiredTool`/`provider`/`ollamaModel`은 API 필드만 있고 미구현 — 7.6절) | `@AiTool` 등록 체계, `config.ConfigTool`, `ChatService.getSpec()`의 `toolsEnabled`, 샘플 `DateTimeTools`, SQL 검증 `SqlSyntaxTools` |
...
- `ChatService.java` 상단에 `AnthropicChatOptions`/`com.anthropic.models.messages.ToolChoice`/`ToolChoiceTool` import가 남아 있어서 언뜻 tool_choice 강제 로직이 있는 것처럼 보이지만, `getSpec()` 본문 어디에서도 쓰이지 않는 죽은 import다(`AiProvider.fromPropertyValue(providerId)`로 provider 유효성만 검증하고, 그 결과인 `provider` 지역 변수도 이후 다시 쓰이지 않는다).
