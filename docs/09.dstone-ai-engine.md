...
- **Workflow의 조립과 step 사이의 데이터 흐름은 100% YAML로 한다.** step은 `input`에 무엇을 받을지(`{{ ... }}` 참조)를,
  `output`에 data를 어떤 모양으로 낼지를 선언한다. 모든 데이터는 실행 컨텍스트 트리 하나를 거쳐 오간다.
...
│   ├── step/
│   │   ├── StepRunner                     공통 인터페이스
│   │   ├── StepInput                      run()의 입력(text | arguments, workflowInput) - 템플릿은 이미 채워진 상태
...
같은 step을 `CompletableFuture`로 동시에 실행한다. 리스트는 요청 값(`input.sqlList`)이든 앞 step 결과(`steps.list.data.lines`)든
상관없고, 항목 개수를 YAML에 미리 적을 필요가 없다. 각 반복은 자기 항목을 `{{item}}`(`itemVariable`로 이름 변경 가능)으로 받는다.

- 반복별 결과는 `steps.<id>.items[i]`에 `{input, text, data, error}`로 순서대로 쌓인다.
...
<input>
WorkFlowExecution[...], StepDefinition[id=extract, type=AGENT, ...], StepInput[text=..., arguments=null, workflowInput={message=...}]
|--------------------------------------------------------------------------------------------------------------------------------------|
...
"무엇을 받을지"를 `{{ ... }}` 참조로, `output`에 "data를 어떤 모양으로 낼지"를 선언한다. 템플릿은 step 종류와 상관없이
...
```yaml
input:                     # 실행 요청이 넣은 값. 시작할 때 한 번 채워지고 바뀌지 않는다
...
- **`StepInput(text, arguments, workflowInput)`** — 엔진이 채워서 넘긴다. AGENT류/APPROVAL은 `text`, TOOL은 `arguments`를 쓰고,
  `workflowInput`(컨텍스트의 `input`)은 Agent system prompt의 `{변수}`를 채우는 데 쓴다.
- **`StepOutcome`** — `Success(text, data)` / `Routed(text, data, route)` / `Failure(text, failureReason)` / `Pending()`.
  엔진은 이것을 컨텍스트에 `{input, text, data, error}`로 남긴다(`error` = `failureReason`).

| StepType | `input` 모양(생략 시) | `output` | `text` | `data` |
|---|---|---|---|---|
| AGENT | 문자열 (`{{previous.text}}`) | 없음 | LLM 답변 원문 | `{}` |
| AGENT | 문자열 (`{{previous.text}}`) | `schema` | data의 JSON 글자 | schema대로 읽은 값 |
...
- **실패 사유는 텍스트에 덧붙이지 않는다.** SUPERVISOR/APPROVAL/TOOL 모두 사유를 `error`(승인 코멘트는 `data.comment`)에만 남긴다.
  필요한 step이 `{{steps.judge.error}}`, `{{steps.review.data.comment}}`처럼 명시적으로 읽는다.
- **SUPERVISOR/ROUTER/APPROVAL은 관문이다.** 받은 input을 결과 텍스트로 그대로 넘기므로, 뒤 step이 input을 생략해도
  (`{{previous.text}}`) 관문 앞 step의 결과를 받는다.
- **TOOL의 결과 텍스트는 항상 Tool 응답 원문이다.** "검증을 통과한 값"이 필요하면 그 값을 만든 step(`{{steps.convert.data.sql}}`)이나
...
|---|---|
| 시스템 프롬프트 | `AgentDefinition.prompt` — `{변수}`는 컨텍스트의 `input`(요청의 message/variables)으로 채움 |
...
|---|---|---|
| `runtime.tool.ToolOutcome(success, message)` (권장) | `success` 필드 | (판정용이라 data 불필요) |
...
- **문서 본문과 승인 코멘트가 섞이지 않는다.** 승인 코멘트는 `data.comment`에 따로 있고, 다음 AGENT가 input의 `[검수 의견]` 칸으로 따로 받는다.
...
- 구조화된 data를 내고 싶으면 `ToolOutcome` 대신 원하는 record/Map을 돌려주고, YAML에서 `output.parse: json`을 적는다.
...
- `message`는 필수이고 `input.message`가 된다. `variables`의 각 값은 `input.<이름>`이 된다.
