...
│   │   ├── WorkFlowDefinition, StepDefinition, StepType(AGENT/TOOL/SUPERVISOR/APPROVAL/ROUTER)
...
│
├── runtime/
│   ├── status/                            StepStatus, StepResult, StepInput, StepOutput, StepFlow, Verdict, RouteDecision, StepPayload,
│   │                                       ToolOutput, WorkFlowExecutionStatus (상태/결과를 나타내는 순수 타입만 모아둔 패키지 - [6절](#6-step-inout-완전-참조--workflow--step--agentapprovaltool--llm)이 이 패키지 전체를 다룬다)
│   ├── workflow/
│   │   ├── WorkFlowContext, WorkFlowExecutor.java   순차/분기(2지선다·ROUTER 다지선다)/병렬(forEachVariable)/루프/승인대기 상태 기계
│   │   └── execution/                     WorkFlowExecution, StepHistoryEntry, WorkFlowExecutionStore(JdbcTemplate)
│   ├── agent/AgentExecutor.java           Agent 하나를 실제로 호출(ChatClient 요청 조립)하는 유일한 지점. call()(자유 텍스트)/
│   │                                       callForEntity(Class)(구조화 응답 - Verdict/RouteDecision/StepPayload 공통 통로)/stream()
│   ├── step/                              StepRunner(공통 인터페이스), AgentStepRunner(AGENT/SUPERVISOR/ROUTER), ToolStepRunner, ApprovalStepRunner
...
sequenceDiagram
    ...
    loop 스텝(또는 forEach 반복)마다
        ...
        SR-->>WE: StepOutput(result, primaryText, data, ...)
        WE->>ST: update() + appendHistory() — 즉시 영속화
    end
    WE-->>WS: SUCCESS/FAIL/WAITING_APPROVAL 중 하나에 도달한 최종 WorkFlowExecution
    WS-->>WC: 최종 상태
...
`WAITING_APPROVAL`(APPROVAL 스텝이 아직 결정을 못 받아 실행 중단). `maxIterations`를 비워두면
`Constants.WorkFlow.DEFAULT_MAX_ITERATIONS`(현재 **10**)이 대신 쓰인다.

**성공/실패 판정 관례**: AGENT step은 `structuredOutput`이 `false`(기본값)면 항상 성공. TOOL step은
`ToolOutput(success, message)`(구조화 값, 권장)를 우선으로 보고, Tool이 그 타입을 반환하지 않으면
응답 텍스트가 `"실패"`로 시작하는지(하위호환)로 판정한다. SUPERVISOR step은 구조화 출력
(`runtime.status.Verdict(pass, reason)`)으로 `verdict.pass()`가 명시적으로 `true`일 때만 성공(그 외
전부 fail-closed). APPROVAL step은 사람의 결정(`approved`)으로 성공/실패가 갈린다. AGENT step에
`structuredOutput: true`를 주면 자유 텍스트 대신 `runtime.status.StepPayload(primaryText, data)`로
...
(`runtime.status.RouteDecision(route, reason)`)으로 LLM이 `StepDefinition.routes`(route 이름 →
...
**data 네임스페이싱**: `StepOutput.data()`는 그 값을 낸 step의 `id`를 접두사로 붙여
...
<output>
StepOutput[result=SUCCESS, primaryText=..., data={sql=...}, failureReason=null, route=null]
|--------------------------------------------------------------------------------------------------------------------------------------|
```
...
flowchart TD
    ...
    LLM -->|자유 텍스트| ASR
    LLM -->|"JSON entity(Verdict/RouteDecision/StepPayload)"| ASR
    ...
    TOOL -->|"String 또는 ToolOutput{success,message} JSON"| TE --> TSR

    APR -->|"variables['approvals'][stepId] 조회"| DECIDE{"결정 있음?"}
    DECIDE -->|"아직 없음"| PENDING["StepOutput.pending()"]
    DECIDE -->|"있음(approved:true/false)"| APR

    ASR --> SO["StepOutput{ result, primaryText, data, failureReason, route }"]
    TSR --> SO
    APR --> SO

    SO --> MERGE["WorkFlowExecutor: namespaced(stepId, data) → variables 병합\nprimaryText → variables['__previous']"]
    MERGE --> NEXT["decideTransition() → 다음 step / SUCCESS / FAIL / LOOP"]
...
| 필드 | 타입 | 의미 | 채워지는 경우 |
|---|---|---|---|
| `result` | `StepResult`(`SUCCESS`\|`FAILURE`\|`PENDING`) | 이 step의 판정 결과 | 항상 |
| `primaryText` | `String` | 다음 step의 `{previous}` 자리로 그대로 이어지는 주 텍스트 | `PENDING`이 아니면 항상 |
| `data` | `Map<String,Object>` | `{stepId.키}`로 네임스페이싱되어 `variables`에 합쳐질 구조화 값 | `structuredOutput:true` AGENT, forEach 반복 결과. 그 외에는 보통 빈 Map |
| `failureReason` | `String` | 실패 사유 | `FAILURE`일 때만, 그 외 `null` |
| `route` | `String` | LLM이 고른 route 이름 | ROUTER일 때만, 그 외 `null` |

```text
StepOutput 팩토리 메서드(runtime.status.StepOutput):
  success(text)                → SUCCESS, data={}, route=null            (AGENT 기본, TOOL 성공, APPROVAL 승인)
  successWithData(text, data)  → SUCCESS, route=null                     (structuredOutput=true AGENT)
  failure(text, reason)        → FAILURE                                 (TOOL 실패, SUPERVISOR 반려, APPROVAL 반려)
  pending()                    → PENDING, primaryText=null                (APPROVAL 미결정)
  routed(text, data, route)    → SUCCESS이지만 route 값을 함께 실어 나름   (ROUTER)
```
...
sequenceDiagram
    ...
    ASR-->>WE: StepOutput.success(answer)  ← 이 step은 항상 SUCCESS
```
...
sequenceDiagram
    ...
    participant AE as AgentExecutor.callForEntity(..., StepPayload.class)
    participant LLM as ChatClient(LLM)
    ASR->>AE: callForEntity(agent, sessionId, caller, variables, renderedText, StepPayload.class)
    AE->>LLM: .user(renderedText).call().entity(StepPayload.class)
    Note over LLM: Spring AI가 {primaryText, data} JSON 스키마를\n프롬프트에 자동으로 끼워 넣고, 응답을 그 스키마로 파싱
    LLM-->>AE: StepPayload{primaryText, data}
    AE-->>ASR: StepPayload
    alt 파싱 성공 & primaryText 있음
        ASR-->>ASR: StepOutput.successWithData(payload.primaryText(), payload.data())
    else 파싱 실패 또는 primaryText 없음
        ASR-->>ASR: StepOutput.failure(renderedText, "...해석하지 못했습니다...")
    end
```

- **IN**: ①과 동일(`renderedText`).
- **OUT**: `StepPayload{primaryText, data}` — `data`의 각 키는 `{이_step_id.키}`로 다음 step이
...
sequenceDiagram
    ...
    alt verdict.pass() == true
        ASR-->>ASR: StepOutput.success(renderedText)
    else pass == false 또는 파싱 실패
        ASR-->>ASR: StepOutput.failure(renderedText + "\n\n[검토 결과] 실패: " + reason, reason)
    end
```
...
sequenceDiagram
    ...
    ASR-->>ASR: StepOutput.routed(renderedText, {}, decision.route())  — 이 step 자체는 항상 SUCCESS
...
- **IN**: ①과 동일. Agent의 `prompt`가 "route로 쓸 수 있는 이름표가 무엇인지"를 직접 안내해야
  ...
  유효한지는 모른다.
- **OUT**: `route` 필드가 채워진 `StepOutput`. 유효성 검증(그 `route`가 `StepDefinition.routes`에
...
sequenceDiagram
    ...
    alt toolResult가 ToolOutput{success,message} JSON으로 파싱됨
        TSR->>TSR: outcome.success() 값으로 판정(권장 경로)
    else 평범한 문자열
        TSR->>TSR: "실패"로 시작하는지로 판정(하위호환 경로)
    end
    alt 성공
        TSR-->>WE: StepOutput.success(normalized)  ← Tool의 응답 문구가 아니라 "검증받은 원본 값"을 그대로 전달
    else 실패
        TSR-->>WE: StepOutput.failure(normalized + "\n\n[검증 결과] " + reason, reason)
    end
```
...
sequenceDiagram
    ...
    alt approved == true
        APR-->>WE: StepOutput.success(renderedText + "\n\n[승인] " + approver + " - " + comment)
    else approved == false
        APR-->>WE: StepOutput.failure(renderedText + "\n\n[반려] " + approver + " - " + reason, reason)
    end
    end
```
...
- `callForEntity(..., Class<T>)`(②③④, structuredOutput AGENT/SUPERVISOR/ROUTER) → Spring AI가
  `T`(`StepPayload`/`Verdict`/`RouteDecision`)의 필드로 JSON 스키마를 자동 구성해 프롬프트에
  끼워 넣고, 응답을 그 스키마로 파싱한 **구조화 객체**. 스키마를 어기면 여기서 예외가 나고, 그
  예외는 항상 호출한 `AgentStepRunner`의 메서드(②③④)가 잡아서 `StepOutput.failure(...)`로 바꾼다.
...
|---|---|---|
| `runtime.status.ToolOutput(Boolean success, String message)` (권장) | `success` 필드를 그대로 사용 | `tools.sql.SqlSyntaxTool`, `tools.jenkins.JenkinsTriggerBuildTool` |
...
flowchart TD
    ...
    P4 -.반려.-> FAIL4(["FAIL"])
    T1["trigger-jenkins : TOOL\nref=triggerBuild\ninputTemplate={jobName:testApp}"] -->|ToolOutput.success| SUCCESS(["SUCCESS"])
    T1 -.실패.-> FAIL5(["FAIL"])
...
| `trigger-jenkins` | TOOL | (직접 안 씀, `inputTemplate`이 고정 JSON) | `tools.jenkins.JenkinsTriggerBuildTool.triggerBuild("testApp")` — LLM 없이 직접 호출 | `ToolOutput.pass("...빌드를 큐에 등록했습니다...")` → `SUCCESS`로 Workflow 종료 |
...
섞인 긴 문서라서, 이를 JSON(`StepPayload.data`) 하나에 완벽하게 이스케이프해서 담으라고 강제하면
...
- **MCP 클라이언트** — `common.config.ConfigMcp`가 `resources/mcp/*.yml`(`McpServerDefinition`,
  ...
  MCP 서버의 Tool을 가져다 쓰는 것)만 다룬다. 2026-09-22 현재 `resources/mcp/`에 등록된 서버 정의는
  없다(빈 상태로 기동이 깨끗하게 통과하는 것만 확인됨, [14절](#14-검증-상태) 참고).
...
동일한 패턴). 파일명이 곧 agent 이름이고, 프롬프트는 파일 안에 직접 적는다. 파일이 많아지면
`resources/agents/testApp/*.yml`처럼 서브 디렉토리로 나눠서 관리해도 된다(로더가 `**/*.yml`로
재귀 탐색한다 - [4절](#4-정의definition--실행runtime-흐름) 참고).

```yaml
agent:
  name: sample-basic-echo-agent   # 파일명(sample-basic-echo-agent.yml)과 맞춘다
...
`resources/mcp/**/*.yml` - 파일 하나에 MCP 서버 하나(`mcpServer:` 최상위 키, 2026-09-22 현재
등록된 파일 없음):
...
2026-09-22 현재 두 그룹으로 나뉘어 있다:

- **`agents/sample/`, `workflows/sample/`** - 엔진 기능 하나하나를 최소 구성으로 확인해보기 위한
  9개 Agent + 13개 Workflow 데모 세트(파일명이 전부 `sample-` 접두어).
...
| Workflow → Step → Agent/Approval/Tool → LLM의 IN/OUT을 다루는 전용 절 없음(여러 절에 흩어져 서술) | [6절](#6-step-inout-완전-참조--workflow--step--agentapprovaltool--llm) 신설 - `StepInput`/`StepOutput`부터 StepType별 시퀀스 다이어그램, `AgentExecutor`가 LLM에 실제로 보내는 7단계 조립 과정, `{previous}`/`{변수명}`/`{stepId.키}` 세 토큰의 유효 범위까지 한 절에서 집중적으로 다룸 |

### 2026-09-18 → 2026-09-20 (구조화 계약 + 다지 분기/동적 팬아웃)

| 이전 | 지금 |
|---|---|
| AGENT step은 항상 자유 텍스트, `StepOutput.data()`를 채우는 러너가 없어 사실상 미사용 | `StepDefinition.structuredOutput: true`로 `runtime.status.StepPayload(primaryText, data)` 구조화 응답 지원 |
| TOOL step 성공/실패 = 응답 텍스트가 `"실패"`로 시작하는지(매직 스트링, `Constants.Outcome.FAIL_PREFIX`) | `runtime.status.ToolOutput(success, message)`(구조화 값) 우선, 옛 접두사 컨벤션은 하위호환으로 유지 |
| `StepOutput.data()`를 `variables.putAll()`로 무조건 병합(같은 키를 반환하는 형제 step이 있으면 조용히 덮어씀) | `WorkFlowExecutor.namespaced()`가 그 값을 낸 step의 `id`를 접두사로 붙여 `{stepId.키}`로 병합 |
| 분기는 `onSuccess`/`onFailure` 2지선다뿐(3갈래 이상은 SUPERVISOR를 여러 겹 쌓아야 함) | `StepType.ROUTER` + `runtime.status.RouteDecision(route, reason)` + `StepDefinition.routes`로 임의 개수 분기를 한 step으로 표현 |
