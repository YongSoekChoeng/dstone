...
>
> ⚠️ **2026-09-14 추가 변경 — `ChatRequest`가 `promptName` 필드를 잃고 `capability`가 필수가 됐다.** `ChatRequest`에서 독립 필드였던 `promptName`이 통째로 사라졌고, 남은 `capability` 필드 하나가 그 자리를 대신한다 — `ChatController.validateMessage()`가 `message`뿐 아니라 `capability`도 비어있으면 `400`을 던지므로, **이제 모든 `/api/ai/chat` 요청은 capability를 반드시 실어야 한다.** 그런데 이 리팩터링 과정에서 `ChatService.buildSpec(sessionId, caller, ChatRequest)`가 `capability.CapabilityRegistry.resolve()`로 찾아낸 `CapabilityDefinition`의 `promptName()`/`toolsEnabled()`/`ragEnabled()`를 전혀 읽지 않고, **요청에 실린 `capability` 문자열 그 자체를 프롬프트 템플릿 이름으로 곧장 사용**하도록 되어 있다(`request.capability()`를 `buildSpec(...)`의 `promptName` 인자 자리에 그대로 넘김). 즉 지금 코드는 "`capability` 이름 == `prompt-name` 값"인 경우에만 우연히 의도대로 동작하고, 두 값이 다르게 설정된 capability(예: `name: sql-helper`, `prompt-name: generic-sql`)는 존재하지 않는 `sql-helper` 템플릿을 찾다가 `IllegalArgumentException`을 던질 것으로 보인다 — `CapabilityDefinition.toolsEnabled()`/`ragEnabled()`도 마찬가지로 단일-호출 경로에서는 사용되지 않고, 대신 `boolean ragEnabled = ...ifEmpty(request.ragEnabled(), "false")`/`boolean toolsEnabled = ...ifEmpty(request.toolsEnabled(), "true")`처럼 요청 값(없으면 하드코딩된 기본값)만 본다. 7.7절 예시처럼 `name`과 `prompt-name`을 항상 같은 문자열로 맞춰 쓰면 겉으로는 정상 동작하므로 지금까지 실동작 검증(8.6절 등)에서는 드러나지 않았다 — **이 문서는 코드의 현재 동작을 그대로 적은 것이고, 이게 의도한 설계인지 고쳐야 할 버그인지는 확인이 필요하다.** Process 경로(`ProcessExecutor.runAgent()` → `agent.AgentDefinition`)는 이 문제와 무관하다 — `AgentDefinition.promptName()`은 `ProcessExecutor`가 직접 읽어서 정상적으로 쓰인다(15~16절 참고).
...
flowchart LR
    ...
    Svc -->|"capability 값을<br/>템플릿 이름으로 그대로 사용<br/>(⚠️ 4절 2026-09-14 노트)"| Prompt["PromptTemplateRegistry<br/>classpath:prompts/{name}/{ver}.st"]
...
```java
// api/dto/ChatRequest.java — 필드는 있다 (⚠️ promptName은 더 이상 없다 - 4절 2026-09-14 노트 참고,
// capability 하나가 그 역할까지 겸한다)
...
- ⚠️ **2026-09-14부터 `capability`는 선택이 아니라 필수다.** `ChatRequest.promptName` 필드 자체가 없어졌고, `ChatController.validateMessage()`가 `capability`가 비어있으면 `message`와 동일하게 `400`으로 막는다 — "capability를 아예 모르는 기존 채팅 요청"이라는 개념 자체가 더 이상 없다.
- ⚠️ **capability의 `prompt-name`/`tools-enabled`/`rag-enabled` 설정값은 단일 호출 경로에서 실제로 읽히지 않는다(4절 2026-09-14 노트에 상세 설명).** `ChatService.buildSpec(sessionId, caller, ChatRequest)`는 `CapabilityRegistry.resolve()`가 돌려준 `CapabilityDefinition`을 processName 확인 용도로만 쓰고, 프롬프트 템플릿은 **요청의 `capability` 문자열 그대로**(`PromptTemplateRegistry.render(request.capability(), ...)`)를 사용한다 — 그래서 지금은 `name`과 `prompt-name`을 항상 같은 값으로 맞춰 둬야 실제로 그 템플릿이 렌더링된다. `tools-enabled`/`rag-enabled`도 마찬가지로 무시되고, 대신 요청의 `toolsEnabled`/`ragEnabled`(비어있으면 각각 `true`/`false` 하드코딩 기본값)만 적용된다.
...
```bash
# 일반 채팅 (Phase 0/1) - ⚠️ capability는 이제 필수다(4절 2026-09-14 노트).
# 아래 예시의 sample-chat처럼 name==prompt-name인 capability를 하나 등록해 두고 쓴다.
curl -X POST http://localhost:8081/api/ai/chat \
  -H "Content-Type: application/json" \
  -d '{
        "message": "안녕, 짧게 인사만 해줘.",
        "sessionId": null,
        "capability": "sample-chat",
        "variables": null,
        "ragEnabled": false,
        "toolsEnabled": false
      }'
# → {"message":"안녕하세요! 😊","provider":"anthropic","sessionId":"3fde76a9-...","capability":"sample-chat"}
```
...
curl -X POST http://localhost:8081/api/ai/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "우리 RAG 파이프라인에서 임베딩 모델로 뭘 쓰는지 알려줘.", "capability": "sample-chat", "ragEnabled": true}'
...
curl -X POST http://localhost:8081/api/ai/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "지금 정확한 날짜와 시간이 몇 시야?", "capability": "sample-chat", "toolsEnabled": true}'
...
| `capability` | String | ⚠️ **필수(2026-09-14부터, 4절 노트)** — 비어있으면 `message`와 마찬가지로 `400 Bad Request`. `CapabilityRegistry`에 등록된 이름이어야 하고, `allowedCallers` 화이트리스트에 caller가 없으면 에러(8.6절). 시스템 프롬프트는 이 문자열 자체를 템플릿 이름으로 써서 `PromptTemplateRegistry`가 렌더링한다(7.7절 — `CapabilityDefinition.promptName()` 설정값이 아니라 이 요청 문자열이 실제로 쓰인다) |
| `variables` | Map | 시스템 프롬프트 템플릿 렌더링용 변수 |
| `ragEnabled` | Boolean | `true`면 `QuestionAnswerAdvisor`로 검색 결과 자동 삽입 (RAG 꺼져 있으면 `IllegalStateException`). 비우면 `false` |
| `toolsEnabled` | Boolean | `true`면 `ConfigTool`에 등록된 모든 Tool을 ChatClient에 연결(Phase 3) — 실제 호출 여부는 LLM 판단(`tool_choice=auto`). 비우면 `true`(기본값) |
