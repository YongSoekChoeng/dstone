...
│   └── consts/AiProvider.java       # anthropic | openai | ollama (예전 gateway 패키지에서 이동)
...
│       ├── SqlSyntaxTools.java             # JSQLParser 순수 문법 검사 - 항상 존재
│       └── PostgresExplainValidationTools.java  # 실제 PostgreSQL EXPLAIN 검증 - dstone.ai.rag.enabled=true일 때만(pgvector용 DataSource 재사용)
├── governance/                      # Phase 4 — Guardrail, PII 필터, rate limit, 비용 트래킹, 인증
│   ├── auth/                        # ✅ 구현됨 — API Key 인증(@Order(1), Servlet Filter)
...
│   ├── ratelimit/                   # ✅ 구현됨 — caller별 요청 제한(@Order(2), auth 뒤에서 실행, Servlet Filter)
...
│   └── guardrail/                   # ✅ 구현됨 — PII 탐지/마스킹(ChatClient CallAdvisor, 다른 종류의 Guardrail은 아직 예정)
...
└── observability/                   # Phase 4 — 토큰 사용량/비용/트레이싱/Eval
    └── usage/                       # ✅ 구현됨 — 사용량/비용/지연시간 로깅(Eval 로깅은 아직 예정)
...
timeline
    ...
    Phase 4 (진행중) : Governance - API Key 인증(완료) : rate limit(완료) : PII Guardrail(완료) : Observability - 사용량/비용 로깅(완료) : sensitive-word Guardrail·Eval(예정)
```
...
| 3 | ✅ 완료 (실동작 검증됨, `requiredTool`은 미검증) | `@AiTool` 등록 체계, `config.ConfigTool`, `ChatService.getSpec()`의 `toolsEnabled`/`requiredTool`(tool_choice 강제), 샘플 `DateTimeTools`, SQL 검증 `SqlSyntaxTools`/`PostgresExplainValidationTools` |
| 4 | 🚧 진행 중 (`governance.auth`/`governance.ratelimit`/`governance.guardrail`/`observability.usage` 완료, 전부 실동작 검증됨) | `governance.auth` — API Key 인증. `governance.ratelimit` — caller별 요청 제한. `governance.guardrail` — PII 탐지/마스킹. `observability.usage` — 토큰 사용량/비용/지연시간 로깅. 남은 것: sensitive-word 등 다른 Guardrail, Eval 결과 로깅 |
...
```mermaid
flowchart LR
    A["@AiTool 클래스<br/>(DateTimeTools, SqlSyntaxTools, PostgresExplainValidationTools)"] -->|ApplicationContext.getBeansWithAnnotation| B["ConfigTool.discover()"]
...
기동 로그에서도 등록된 Tool 목록을 바로 확인할 수 있다(아래는 Agentic RAG `RetrievalTools`가 있던 시점에 실제로 캡처했던 로그 — `searchKnowledgeBase`는 7.5절에서 설명한 재설계로 지금은 없고, 대신 `validateSqlSyntax`/`validatePostgresSyntax`가 등록된다):
```
...
`prompts/oracle-to-postgresql/v1.st`처럼 **LLM이 SQL을 생성**하는 프롬프트에서, 최종 답변을 내놓기 전에 스스로 문법을 검증하고 틀렸으면 고치는 "자기수정 루프(self-correction loop)"에 쓰라고 만든 Tool 두 개다. 둘 다 `tools/sql/` 패키지에 있고, 검증 정확도와 가용성이 서로 다른 트레이드오프를 갖는다.

```mermaid
flowchart LR
    LLM(["LLM이 생성한 SQL"]) --> S1{"1차: SqlSyntaxTools<br/>(JSQLParser, 순수 문법)"}
    S1 -->|실패| Fix1["LLM이 오류 메시지 보고<br/>SQL 재작성"] --> S1
    S1 -->|통과| S2{"2차(선택): PostgresExplainValidationTools<br/>(실제 PostgreSQL EXPLAIN)"}
    S2 -->|실패| Fix2["LLM이 오류 메시지 보고<br/>SQL 재작성"] --> S1
    S2 -->|통과| Done(["최종 SQL 답변"])
    style S2 stroke-dasharray: 5 5
```

| Tool | 존재 조건 | 검증 방식 | 장단점 |
|---|:---:|---|---|
| `SqlSyntaxTools.validateSqlSyntax` | 항상 존재 | JSQLParser로 순수 문법 구조만 검사(특정 DB 서버 불필요) | 어떤 대상 스키마든 항상 쓸 수 있지만, PostgreSQL 고유 문법의 세부까지는 완벽히 검증하지 못함 |
| `PostgresExplainValidationTools.validatePostgresSyntax` | `dstone.ai.rag.enabled=true`일 때만(pgvector용 `DataSource` 재사용) | 실제 PostgreSQL 서버에 `EXPLAIN (COSTS FALSE)`만 실행(계획만 세우고 대상 SQL 자체는 절대 실행 안 함, 방어적으로 트랜잭션도 매번 rollback) | 진짜 PostgreSQL 파서로 검증되지만, 이 서버(`dstone_ai` DB)에 실제 테이블이 없는 게 보통이라 "문법은 맞는데 이 서버 기준으로는 객체 존재를 확인 못 했다"는 부분 통과 결과가 흔함 |

- **왜 두 단계인가**: `SqlSyntaxTools`는 RAG 인프라와 무관하게 항상 쓸 수 있어 1차 필터로 두고, 이 엔진이 RAG(pgvector)용으로 이미 물고 있는 실제 PostgreSQL 서버를 공짜로 재사용해 더 엄격한 2차 검증(`PostgresExplainValidationTools`)을 얹었다 — 별도 인프라 추가 없이 검증 정확도만 높인 구조다. `PostgresExplainValidationTools`가 `dstone.ai.rag.enabled`에 묶여 있는 건 RAG를 껐을 때 실제로 없어지는 게 자연스러운 다른 조건부 빈들(`PostgresExplainValidationTools`의 유일한 생성자 의존성은 항상 존재하는 `DataSource`라서 6.2절의 `RagService` 패턴과 달리 `@ConditionalOnProperty`를 그대로 둬도 안전하다 — DataSource는 어차피 항상 있는 빈이기 때문이다)과 같은 결이다.
- **세미콜론 다중 문장 방어**: 둘 다 JSQLParser로 "정확히 하나의 SELECT/INSERT/UPDATE/DELETE 문"인지 먼저 확인한다 — `EXPLAIN` 뒤에 실제로 실행되는 두 번째 문장을 세미콜론으로 몰래 끼워넣는 것을 막기 위해서다.
- **안전장치(`PostgresExplainValidationTools`)**: `EXPLAIN`(`ANALYZE` 없이)은 대상 SQL을 절대 실행하지 않지만, 그래도 방어적으로 트랜잭션을 열고 끝나면 무조건 `rollback`한다.
...
Phase 4는 `governance`(Guardrail/PII 필터/rate limit/비용 트래킹/인증)와 `observability`(토큰/비용/트레이싱/Eval) 두 패키지를 다룬다. `governance.auth`(API Key 인증, 8.2절)부터 구현했다 — rate limit/비용 트래킹은 "누가 호출했는지"가 먼저 정해져야 의미가 있기 때문이다. 그 첫 소비자로 `governance.ratelimit`(caller별 요청 제한, 8.3절)을 구현했고, 이어서 `governance.guardrail`(PII 탐지/마스킹, 8.4절)과 `observability.usage`(토큰 사용량/비용/지연시간 로깅, 8.5절)까지 구현했다 — "비용 트래킹"은 governance에 별도 예산 강제 기능을 따로 두지 않고 `observability.usage`가 caller별 비용을 로그로 남기는 것으로 갈음했다(아래 8.5절 참고). 남은 것은 Guardrail의 다른 종류(sensitive-word 차단 등)와 Eval 결과 로깅뿐이다.
...
- `dstone.ai.governance.auth.keys`는 YAML 시퀀스(`key`/`caller` 쌍)다. `key`는 DB 패스워드와 동일한 컨벤션으로 `ENC(...)` 암호화를 권장한다. 이 값은 리스트-오브-오브젝트라 `PromptProperties`처럼 `ConfigProperty.getProperty(String)`(단순 스칼라 조회)로는 못 읽어서, `ApiKeyProperties`만 Spring Boot의 `Binder`를 직접 써서 바인딩한다 — `ENC(...)` 복호화는 PropertySource 레벨에서 일어나므로(`net.dstone.common.config.ConfigProperty`) `Binder`로 읽어도 `@ConfigurationProperties`와 동일하게 적용된다.
...
- `ChatService.getSpec()`이 이미 쓰던 `AnthropicChatOptions`(7.6절, `requiredTool`)와는 무관한 별도 계층이다 — API Key 인증은 "누가 호출했는지", `tool_choice` 강제는 "무엇을 호출해야 하는지"를 다룬다.
...
| 4 | `governance.guardrail` | ✅ 완료 (실동작 검증됨, 8.4절 참고) | PII 탐지/마스킹 — `PiiGuardrailAdvisor`/`PiiGuardrailProperties`/`PiiPatterns`(ChatClient CallAdvisor) |
| 4 | `observability.usage` | ✅ 완료 (실동작 검증됨, 8.5절 참고) | 토큰 사용량/비용/지연시간 로깅 — `UsageLoggingAdvisor`/`UsageProperties`(ChatClient CallAdvisor). "비용 트래킹"은 이 기능으로 갈음(8.5절 참고) |
| 4 | `governance` (다른 종류의 Guardrail) | ⏳ 예정 | sensitive-word 차단 등 — Spring AI 기본 제공 `SafeGuardAdvisor`를 `governance.guardrail`에 이어서 붙이는 방식을 검토 |
| 4 | `observability` (Eval) | ⏳ 예정 | Eval 결과 로깅 — 평가 데이터셋/채점 로직 등 이 모듈에 없는 전제가 먼저 필요 |

Phase 3(`tools` 패키지)는 "단순 오케스트레이션"(Spring AI ChatClient의 내장 tool-calling 루프)까지 완료된 상태다. 여러 Tool을 사람이 미리 정한 순서로 묶어 실행하는 멀티스텝 워크플로우/그래프 엔진처럼 더 복잡한 오케스트레이션이 필요해지면 그건 별도 후속 작업으로 다룬다.
