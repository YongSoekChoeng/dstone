...
> 참고: `dstone-saga`라는 별도 모듈은 없습니다. 이 문서는 `dstone-common`(오케스트레이터/아웃박스 엔진 — `net.dstone.common.messaging.saga.*`, `net.dstone.common.messaging.outbox.*`)과
> `dstone-boot`(그 엔진을 사용하는 샘플 컨트롤러/리스너/스텝 서비스 — `net.dstone.boot.sample.saga.*`, `net.dstone.boot.common.messaging.saga.*`)에 걸쳐 있는 **SAGA 오케스트레이션 기능**을 다룹니다.
...
`OutboxRelayScheduler.relay()` (`OutboxRelayScheduler.java:28`) → `OutboxRelay.dispatchPending(100)` (`OutboxRelay.java:52`)
