...
dstone 멀티모듈 프로젝트의 빌드 명령, 모듈별 산출물, 배포 방식(VM 스타일 / 컨테이너·쿠버네티스)과 CI/CD 파이프라인을 한 곳에 모은 문서다. 각 모듈의 기능/설정 상세는 모듈별 문서([05.dstone-common.md](05.dstone-common.md), [06.dstone-boot.md](06.dstone-boot.md), [07.dstone-batch.md](07.dstone-batch.md), [08.dstone-batchadmin.md](08.dstone-batchadmin.md), [09.dstone-ai-engine.md](09.dstone-ai-engine.md))를, 배포 아키텍처 설계 배경은 [04.cloud-architecture.md](04.cloud-architecture.md)를 참고한다.

## 1. 전제 조건

- JDK 21, Maven 3.9.x — 설치 방법은 [software/01.jdk.md](software/01.jdk.md), [software/02.maven.md](software/02.maven.md)
...
- 이미지 태그는 로컬 사설 레지스트리(`localhost:5000`, kind 클러스터의 containerd에 미러로 등록됨)를 거친다. 레지스트리/클러스터 구성은 [software/11.kubernetes.md](software/11.kubernetes.md) 참고.
...
- ⚠️ **`dstone-ai-engine`을 실제 배포하기 전에 확인할 것 (네트워킹)**: RAG(Phase 2)가 의존하는 PostgreSQL(pgvector)과 Ollama가 아직 kind 게이트웨이 IP(`172.18.0.1`)로 열려있지 않다(dstone-boot이 겪었던 MySQL/Redis 게이트웨이 IP 조치가 이 둘에는 아직 적용 안 됨) — 상세: [04.cloud-architecture.md 7절](04.cloud-architecture.md#7-알려진-한계--후속-과제).
- ⚠️ **`dstone-ai-engine`을 실제 배포하기 전에 확인할 것 (ConfigMap 드리프트)**: `dstone-ai-engine/k8s/configmap.yaml`은 스켈레톤을 처음 만든 **Phase 0 시점의 스냅샷 그대로** 커밋돼 있고, 그 뒤 실제 `conf/application.yml`이 거친 Phase 1~4 변경이 하나도 반영되지 않았다(이 클러스터에 한 번도 배포된 적이 없어서 아무도 갱신할 계기가 없었다). 지금 그대로 `kubectl apply`하면 최소 아래 문제가 생긴다:
  - `spring.ai.model.chat` 자체가 없다 — anthropic/openai/ollama 스타터가 전부 클래스패스에 있는 상태에서 이 값이 없으면 기동이 실패한다(`conf/application.yml` 주석 참고).
  - `autoconfigure.exclude`가 Boot 4 이전 패키지 경로(`org.springframework.boot.autoconfigure.security.servlet.*`)를 쓰고 있다 — 실제 코드가 쓰는 `org.springframework.boot.security.autoconfigure.*`와 다르므로 Security 자동설정이 exclude되지 않는다.
  - `RedisAutoConfiguration`을 exclude 목록에 넣어 Redis를 꺼둔 채인데, Phase 1(`session`)부터는 Redis가 대화 히스토리 저장에 반드시 필요하다 — 이 상태로 배포하면 세션 기능이 조용히 깨진다.
  - `spring.datasource.*`(PostgreSQL), `spring.ai.model.embedding`/`spring.ai.ollama.*`/`spring.ai.vectorstore.*`(RAG), `dstone.ai.session/prompt/rag/governance/observability` 섹션이 통째로 빠져 있다.
  - → 최초 배포 전에 `dstone-ai-engine/conf/application.yml`을 기준으로 `dstone-ai-engine/k8s/configmap.yaml`을 다시 맞춰야 한다(민감정보는 `ENC(...)`인 값만 그대로 옮기면 되고, `${DB_HOST}` 등 `env-k8s.properties`로 치환되는 값은 실제 게이트웨이 IP로 채워야 한다).

## 7. dstone-batch / dstone-batchadmin — VM 스타일 배포 (`bin/*.sh`)

systemd에 등록하지 않고 순수 쉘 스크립트로만 기동/중지한다(설계 배경: [04.cloud-architecture.md](04.cloud-architecture.md#4-dstone-batch--dstone-batchadmin--vm-스타일)).
...
- Jenkins Controller는 WSL 호스트에 상주하며, `jenkins` 시스템 계정이 `docker` 그룹에 속해 있어 별도 인프라 없이 `docker`/`kubectl`을 실행한다. 설치/권한 설정은 [software/12.jenkins.md](software/12.jenkins.md) 참고.
...
- **`dstone-ai-engine`에서 RAG(Phase 2) 문서 적재 시 `NoClassDefFoundError: ChecksumInputStream`**: `dstone-common`이 직접 고정한 `commons-io 2.15.1`을 Maven이 "nearest wins"로 선택해, `spring-ai-tika-document-reader`가 필요로 하는 `commons-io 2.16.0+`의 클래스가 빠진다 — `dstone-ai-engine/pom.xml`의 `dependencyManagement`에서 이 모듈만 `commons-io`를 상향 고정해 해결했다. 상세: [09.dstone-ai-engine.md 13절](09.dstone-ai-engine.md#13-문제-해결-실제로-겪은-에러-모음).
