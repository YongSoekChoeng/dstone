...
- ⚠️ **`dstone-ai-engine`을 실제 배포하기 전에 확인할 것**: RAG(Phase 2)가 의존하는 PostgreSQL(pgvector)과 Ollama가 아직 kind 게이트웨이 IP(`172.18.0.1`)로 열려있지 않다(dstone-boot이 겪었던 MySQL/Redis 게이트웨이 IP 조치가 이 둘에는 아직 적용 안 됨) — 상세: [04.cloud-architecture.md 7절](04.cloud-architecture.md#7-알려진-한계--후속-과제).
