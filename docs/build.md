...
## 설정 프로파일 (`-Dspring.profiles.active=<profile>`)
...
## dstone-batch / dstone-batchadmin — VM 스타일 배포 (`bin/*.sh`)

systemd에 등록하지 않고 순수 쉘 스크립트로만 기동/중지한다(설계 배경: [cloud-architecture.md](cloud-architecture.md#dstone-batch--dstone-batchadmin--vm-스타일)).
