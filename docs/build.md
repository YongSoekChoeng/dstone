...
- **`dstone-ai-engine`에서 RAG(Phase 2) 문서 적재 시 `NoClassDefFoundError: ChecksumInputStream`**: `dstone-common`이 직접 고정한 `commons-io 2.15.1`을 Maven이 "nearest wins"로 선택해, `spring-ai-tika-document-reader`가 필요로 하는 `commons-io 2.16.0+`의 클래스가 빠진다 — `dstone-ai-engine/pom.xml`의 `dependencyManagement`에서 이 모듈만 `commons-io`를 상향 고정해 해결했다. 상세: [dstone-ai-engine.md 13절](dstone-ai-engine.md#13-문제-해결-실제로-겪은-에러-모음).
