...
sequenceDiagram
    ...
    Svc->>Engine: WebClient(timeout 120s) POST /api/ai/chat<br/>{message: originalSql, sessionId: 새 UUID, capability: "oracle-to-postgresql"}
...
- **타임아웃 120초**: `getWebClient(120)` - 문법 검증 Tool 자기수정 루프(09.dstone-ai-engine.md 7.5절)가 붙어서 일반 채팅(기본 60초)보다 오래 걸릴 수 있어 여유를 더 뒀다.
