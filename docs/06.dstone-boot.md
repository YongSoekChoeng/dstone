...
- **workflowId를 고를 때마다 message/variables에 기본 샘플값이 자동으로 채워진다.** 샘플값은
  `ai/assets/js/workflow.js`의 `WORKFLOW_SAMPLES`(workflowId → message/variables/note)에 있고, 각 Workflow
  YAML 맨 위 주석의 "Workflow 테스트" 예시와 같은 값이다(이미 입력해 둔 값은 덮어쓴다). 주의사항(`note`)은
  message 아래에 안내 문구로 보여준다. 여기에 없는 workflowId는 입력칸을 비워 두므로, `dstone-ai-engine`에
  새 Workflow YAML을 추가했다면 이 맵에도 한 줄 추가해 두면 된다.
