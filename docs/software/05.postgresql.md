...
## 6. dstone 프로젝트에서의 역할
`dstone-ai-engine`의 RAG(Phase 2) VectorStore가 이 인스턴스를 쓴다 - `net.dstone.ai.api.service.RagService`가 청크로 쪼갠 문서를 [Ollama](14.ollama.md)로 임베딩한 뒤 pgvector 테이블에 저장하고, 같은 클래스가 유사도 검색도 담당한다(상세: [09.dstone-ai-engine.md 6절](../09.dstone-ai-engine.md#6-phase-2--rag-retrieval-augmented-generation)). 전용 롤/DB/확장을 아래처럼 한 번 생성해뒀다(최초 1회, `postgres` 슈퍼유저 권한 필요):
