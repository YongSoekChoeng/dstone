...
- 설치된 모델:
  ...
  - `llama3.2` (3B) - [Ollama Web UI Lite](15.ollama-webui-lite.md)에서 수동 채팅 테스트용으로 추가. CPU 전용 + 가용 메모리 제약(`.wslconfig`상 이 WSL 환경 전체 할당 8GB 중, MySQL/PostgreSQL/Kafka/Jenkins 등을 함께 띄우면 `free -h` 기준 가용량이 2GB 안팎까지 줄어듦 - [02.environment.md 6.4절](../02.environment.md#64-대상-pc-준비) 참고) 때문에 가벼운 모델로 골랐다 - `dstone-ai-engine`은 이 모델을 쓰지 않는다(채팅은 Anthropic Claude가 담당). 실제로 한국어 자유 응답을 시켜보면 3B급 소형 모델의 한계로 언어가 뒤섞이거나(예: 한국어 문장 중간에 영어/이탈리아어 단어가 공백 없이 끼어드는 등) 품질이 들쭉날쭉할 수 있다 - 이는 버그가 아니라 파라미터 규모의 한계이며, 더 나은 한국어 응답이 필요하면 `exaone3.5`/`qwen2.5` 등 한국어·다국어 비중이 더 높은 모델로 교체하는 것을 고려한다.
