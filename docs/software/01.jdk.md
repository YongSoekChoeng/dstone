...
> 💡 **한 줄 요약**: 5개 모듈 전부가 이 JDK 21 하나로 컴파일·실행된다 — 버전을 올리거나 다른 JDK와 섞어 쓰면 `pom.xml`의 `<maven.compiler.source>21</maven.compiler.source>`부터 어긋나기 시작한다([6절](#6-트러블슈팅) 참고).
...
- **`mvn clean package`가 "invalid target release: 21" 등으로 실패**: `mvn -version`의 "Java version" 값이 21 미만이면 발생 — [02.maven.md 5절](02.maven.md#5-설정)의 JDK 버전 일치 항목 참고.
