...
/**
 * dstone-ai-engine 전역에서 쓰는 static 상수를 한 곳에 모아둔다. 원래 각 클래스에 local로 있던
 * static final 필드들을 여기로 옮긴 것으로, 어느 클래스가 원래 주인이었는지 nested class로
 * 구분해뒀다(같은 이름의 PREFIX가 여러 클래스에 있었지만 nested class 덕분에 서로 안 섞인다).
 */
