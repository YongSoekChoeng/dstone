 ...
 * 실행하고 텍스트로 결과를 돌려주는" 로직이다.
 *
 * ProcessBuilder(command)는 셸을 거치지 않고 프로세스를 직접 실행하므로(/bin/sh -c로 감싸지 않음),
 * 인자에 `;`/`&&`/`|` 같은 셸 메타문자가 들어있어도 셸 문법으로 해석되지 않고 단순 인자 문자열로만
 * 전달된다 - LLM이 만든 인자값이라도 명령 주입으로 이어지지 않는 이유다. 다만 command[0](실행 파일
 * 경로) 자체는 호출부가 화이트리스트로 확정한 값이어야 한다(이 클래스는 그 검증을 하지 않는다).
 */
