...
/**
 * resources/agents/*.yml 파일에 등록된 Agent 하나("LLM에게 일을 시키는 단위" - promptName으로 무엇을 시스템 프롬프트로 쓸지, toolsEnabled/ragEnabled로 Tool/RAG를 붙일지를 갖고 있다).
 * 
 * api.controller.ChatController가 request.agent()로 직접 가리킬 수도 있고,  StepType.AGENT/SUPERVISOR step의 ref가 가리킬 수도 있다.
 * 어느 경로로 오든 caller 검증은 이 정의의 allowedCallers로 동일하게 이뤄진다(common.registry.AgentRegistry.resolve() 참고).
 * @param name Agent 이름
