...
/**
 * @AiTool이 붙은 빈들을 기동 시점에 스캔해서 Spring AI ToolCallbackProvider로 묶어준다. - type이 AGENT인 step에서는 어떤 Tool을 언제 호출할지 Spring AI의
 *          ChatClient가 LLM과 대화를 주고받으며 알아서 처리. - type이 TOOL인 step에서는 runtime.tool.ToolExecutor가 이름으로 직접 찾아 LLM 없이 호출.
 *
 *          caller별 Tool 화이트리스트 설정은 여러 앱이 공유하는 엔진에서 한 앱에게만 허용된 Tool을 다른 앱이 붙여 쓰지못하게 막는다. Tool 화이트리스트 설정이 없는 caller는
 *          화이트리스트가 없는 것으로 보고 등록된 Tool 전체를 허용한다.
 */
@Component
public class ConfigTool extends BaseObject {
	...
	/**
	 * "tools" 값을 List 또는 콤마 구분 String 어느 쪽으로 와도 다루기 위한 파싱이다 - caller 화이트리스트 자체는 있는데 tools가 비어 있으면(YAML `[]`) "허용된 Tool 0개"를
	 * 뜻해야 하므로, 빈 문자열도 빈 리스트와 동일하게 처리한다(그냥 무시하면 화이트리스트가 있는지조차 모르는 caller와 똑같이 "전체 허용"이 돼버려서 화이트리스트가 있으나 마나 해진다).
	 * 
