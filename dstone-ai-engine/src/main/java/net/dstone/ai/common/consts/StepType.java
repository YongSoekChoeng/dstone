...
public enum StepType {
	...
	/**
	 * <pre>
	 * AGENT와 마찬가지로 LLM을 호출하지만, 응답을 자유로운 글이 아니라 
	 * "통과했는지 아닌지, 그리고 왜 그런지"를 담은 정해진 형식(Verdict record - pass/reason)으로 받아서 
	 * 그걸로 성공/실패를 판정하는 step입니다.
	 * (자세한 호출 방식은 runtime.agent.AgentExecutor.callForVerdict, 반환값의 구조는 runtime.agent.Verdict 참고)
	 *
	 * 여러 step의 결과물이 괜찮은지 감독하고 다시 검토하는 역할에 씁니다. 
	 * TOOL step처럼 결과가 자바 코드에서 정해진 값으로 나오는 게 아니라 LLM이 자유롭게 만든 글이기 때문에, 
	 * "글이 '실패'로 시작하는가"처럼 단순한 문자열 비교로는 믿을 수 있는 판정을 하기 어렵습니다. 
	 * 그래서 Spring AI의 기능을 활용해서, Verdict가 가져야 할 JSON 형식을 프롬프트에 자동으로 끼워 넣고 
	 ...
	 *
	 * 업무 성격상 세 갈래 이상으로 나뉘어야 하는 경우(예: 문의 내용에 따라 담당 부서를 나누는 경우)에 쓰기 위한 타입입니다. 
