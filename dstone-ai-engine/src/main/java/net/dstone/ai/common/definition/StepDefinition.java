...
/**
 * WorkflowDefinition.steps의 항목 하나. ref의 의미는 type에 따라 다르다: AGENT/SUPERVISOR는 common.registry.AgentRegistry에 등록된 Agent 이름,
 * TOOL은 ConfigTool에 등록된 Tool 이름, RAG는 쓰지 않는다.
 *
 * onSuccess/onFailure를 둘 다 비워두면 "성공했으면 목록상 다음 step, 실패했으면 Workflow 전체 실패"로 동작한다(가장 흔한 순차 실행). 
 * onFailure에 이전 step의 id를 넣으면 루프(예: 검증 실패 시 변환 step으로 되돌아가기)가 되고, 
 * WorkflowDefinition.maxIterations가 무한루프를 막는다. 
 * onSuccess/onFailure에 예약어 "SUCCESS"/"FAIL"을 넣으면 그 자리에서 바로 Workflow를 종료한다.
 *
 ...
 * 
 * @param id step 식별자
 * @param type step이 실제로 무엇을 실행하는지 구분하는 종류
