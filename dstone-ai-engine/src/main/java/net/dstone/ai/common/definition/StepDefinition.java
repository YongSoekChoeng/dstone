 ...
 * onSuccess와 onFailure를 둘 다 비워두면 가장 단순한 동작이 됩니다: 성공하면 목록에서 바로 다음
 * step으로 넘어가고, 실패하면 그 자리에서 Workflow 전체가 실패로 끝납니다(보통 여러 step을 순서대로
 * 쭉 실행할 때 이렇게 씁니다). onFailure에 앞쪽에 있는 다른 step의 id를 적어두면, 실패했을 때 그
 * step으로 되돌아가는 "재시도 루프"가 만들어집니다(예: 검증에 실패하면 값을 고치는 step으로 되돌아가는
 * 경우). 이런 루프가 끝없이 돌지 않도록 WorkFlowDefinition.maxIterations가 전체 실행 횟수를 제한합니다.
 * onSuccess나 onFailure에 "SUCCESS" 또는 "FAIL"이라는 특별한 이름(예약어)을 적으면, 그 자리에서 바로
 * Workflow 전체를 성공/실패로 끝냅니다. type이 ROUTER인 step은 onSuccess/onFailure를 쓰지 않고 대신
 * routes를 씁니다(아래 routes 설명 참고).
 *
 * ## 성공/실패는 어떻게 판정하는가
 ...
 * 찾아서, 그 목록에 들어있는 항목 개수만큼 이 step 하나를 한꺼번에(병렬로) 실행합니다. 예를 들어
 * 목록에 항목이 3개 있으면 이 step이 동시에 3번 실행됩니다. 각 실행에서는 자신이 맡은 항목 값이
 * itemVariable로 지정한 이름(기본값 "item")으로 {item} 이라는 토큰에 채워집니다. 서로 다른 값을
 * 담은 목록을 넘기면 "미리 정해진 개수만큼 동시에 실행"하는 효과도 낼 수 있습니다. 다만 "서로 다른
 * Tool이나 Agent를 동시에 호출"하는 것처럼 반복마다 대상 자체가 달라지는 경우는 표현할 수 없습니다 -
 ...
 *
 * @param id              이 step을 가리키는 이름입니다. 같은 Workflow 안에서 onSuccess/onFailure/routes가
 *                        "다음에 어느 step으로 갈지"를 가리킬 때 이 이름을 씁니다
 * @param type            이 step이 실제로 무엇을 하는지 정하는 값입니다(AGENT/TOOL/SUPERVISOR/APPROVAL/ROUTER)
 * @param ref             type에 따라 Agent 이름 또는 Tool 이름을 가리킵니다. type이 APPROVAL이면 쓰이지 않습니다
 ...
 *                        그 항목 개수만큼 이 step을 동시에 실행합니다(비워두면 한 번만 실행합니다). 각 실행
 *                        안에서는 그 실행이 맡은 항목이 itemVariable(기본값 "item")이라는 이름으로 {item}
 *                        토큰에 채워집니다
 * @param itemVariable    forEachVariable로 여러 번 실행될 때, 각 실행이 맡은 항목을 채워 넣을 변수 이름입니다
 *                        (비워두면 기본값인 "item"을 씁니다)
 */
