...
public enum StepType {
	...
	/**
	 * AGENT와 똑같이 Agent를 호출하지만, 응답 텍스트가 "실패"로 시작하는지로 성공/실패를 가른다(TOOL과
	 * 같은 판정 컨벤션). 여러 step의 결과를 감독/재검토하는 역할이라, Agent의 프롬프트 자체가 "통과: .../실패:
	 * 이유" 형식으로 답하도록 작성돼 있어야 한다(강제 장치는 없다 - 프롬프트 설계 컨벤션이다).
	 */
	SUPERVISOR
