...
public enum StepType {
	 ...
	 * 받아서 성공/실패를 가른다(runtime.agent.AgentExecutor.callForVerdict, runtime.status.Verdict
	...
	/**
	 * 사람의 승인/반려를 기다리는 스텝이다(ref는 쓰지 않음). 첫 실행에서는 아직 결정이 없으니 Workflow
	 * 실행 자체를 WAITING_APPROVAL 상태로 멈추고, 외부에서 결정 API가 호출되면 같은 스텝을 다시 실행해
	 * 이번엔 결정에 따라 성공/실패로 진행한다(runtime.step.ApprovalStepRunner 참고). parallelGroup과는
	 * 함께 쓸 수 없다(common.registry.WorkFlowRegistry가 기동 시 검증).
	 */
	APPROVAL
