...
/**
 * TOOL step이 Tool의 응답 텍스트를 어떤 모양의 data로 정리할지 정하는 값입니다.
 * YAML의 step output.parse 자리에 적습니다(대소문자 구분 없음. 예: parse: lines).
 *
 * 어떤 값을 고르든 step의 text에는 항상 Tool 응답 원문이 그대로 남습니다. 이 값은 "다음 step이
 * {{steps.id.data.키}}로 꺼내 쓸 data를 어떻게 만들지"만 정합니다.
 */
public enum ToolParse {

	/** data를 만들지 않습니다(빈 맵). 아무것도 적지 않았을 때의 기본값입니다. */
	TEXT,

	/**
	 * 응답을 JSON으로 읽어서 data로 씁니다. JSON 객체면 그대로 data가 되고, JSON 배열이면
	 * {items: [...]} 모양으로 감싸서 data가 됩니다. JSON이 아니면 step이 실패합니다.
	 */
	JSON,

	/**
	 * 응답을 줄 단위로 나눠서 {lines: [...]} 모양의 data로 씁니다. 빈 줄은 버립니다.
