...
/**
 * 
 * classpath:prompts/{name}/{version}.st에 있는 프롬프트를 이름과 버전으로 찾아 렌더링해준다. 버전은 PromptProperties에서 프롬프트명별로 override할 수 있고,
 * 없으면 default-version을 쓴다. SI 프로젝트는 이 리소스 파일을 추가/변경하고 설정 한 줄만 고치면 된다 - 코드는 건드릴 필요 없다.
 */
@Component
public class PromptTemplateRegistry extends BaseObject {
	 ...
	 * @param promptProperties 프롬프트 버전 설정을 조회할 객체
	 */
	...
	/**
	 * @param name      렌더링할 프롬프트 이름
	 * @param variables 프롬프트에 채워 넣을 변수 값
	 */
		...
		return this.cache.computeIfAbsent(name + "@" + version, new Function<String, PromptTemplate>()
			{
				...
				public PromptTemplate apply(String key) {
					...
					if (!resource.exists()) {
						throw new IllegalArgumentException("[" + name + "]프롬프트를 찾을 수 없습니다: " + path);
					}
