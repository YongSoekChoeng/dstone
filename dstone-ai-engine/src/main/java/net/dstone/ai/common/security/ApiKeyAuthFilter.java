...
/**
 * dstone.ai.security.auth.enabled=true일 때만 실제로 막고, 꺼져 있으면 아무 영향도 주지 않는다. 이
 * 모듈은 SecurityAutoConfiguration을 통째로 빼뒀기 때문에(conf/application.yml 참고) Spring
 * Security를 다시 가져오는 대신, 헤더 하나만 확인하는 가벼운 Filter로 최소한만 구현했다.
 *
 * OncePerRequestFilter를 구현한 @Component는 Spring Boot가 알아서 서블릿 필터로 등록해준다.
 * /actuator/**는 k8s의 liveness/readiness probe 경로라서 인증 없이 통과시킨다.
 *
 * 같은 패키지의 RateLimitFilter(@Order(2))가 CallerContext에 기록해둔 caller를 그대로 가져다
 * 쓰므로, 이 필터가 반드시 먼저(@Order(1)) 실행돼야 한다.
 *
 * OAuth2 client-credentials 대신 API Key를 고른 이유: 이 모노레포 어디에도 Authorization Server가
 * 없고(dstone-boot의 OAuth2는 소셜 로그인용 client일 뿐 IdP는 아니다), 이 엔진을 부르는 쪽이 정해진
 * SI 프로젝트들끼리의 서비스 간 호출이라 SI 프로젝트마다 키를 하나씩 발급하는 것으로 충분하다.
 */
@Component
@Order(1)
