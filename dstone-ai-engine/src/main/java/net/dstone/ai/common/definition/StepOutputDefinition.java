 ...
 * 다듬을 필요 없이 {{steps.id.data.키}}로 바로 꺼내 쓰면 됩니다.
 *
 * step 종류에 따라 쓸 수 있는 필드가 다릅니다(잘못 섞어 쓰면 엔진이 켜질 때 막힙니다).
 * <pre>
 * - AGENT : schema만 씁니다. LLM이 이 모양의 JSON으로 답하도록 강제하고, 그 JSON이 data가 됩니다.
 ...
 *             parse: lines
 *             pattern: '^\[FILE\] (.+)$'
 * - SUPERVISOR / ROUTER / APPROVAL : output을 쓰지 않습니다. data 모양이 정해져 있습니다
 ...
 * @param parse   TOOL 전용. Tool 응답을 data로 정리하는 방법입니다. 비워두면 TEXT(data 없음)입니다.
