 ...
 * net.dstone.ai.session.RedisChatMemorySession이 Redis에 저장해 관리한다).
 *
 * capability는 필수다(비우면 message 없을 때처럼 400) - "이 요청이 뭘 하려는 건지" 이름 하나로
 * 알려주는 값이고, net.dstone.ai.capability.CapabilityRegistry에 미리 등록해둔 이름이어야 한다.
 * 지금은 이 문자열이 곧 net.dstone.ai.prompt.PromptTemplateRegistry가 찾는 템플릿 이름으로도
 * 그대로 쓰인다 - 그래서 등록해둔 capability의 prompt-name 값은 이 문자열과 같아야 실제로 그
 * 템플릿이 렌더링된다(docs/09.dstone-ai-engine.md 4절/7.7절 참고). 등록 안 된 이름을 보내면 그
 * 자리에서 바로 에러가 난다.
 *
 ...
 * 자리이지만, net.dstone.ai.api.service.ChatService가 이 세 필드를 전혀 읽지 않는다 - 즉 지금은
 * 요청에 넣어 보내도 조용히 무시된다(docs/09.dstone-ai-engine.md 7.6절 참고). 아래 설명은 구현
 * 예정 사양이지 현재 동작이 아니다.
 *
