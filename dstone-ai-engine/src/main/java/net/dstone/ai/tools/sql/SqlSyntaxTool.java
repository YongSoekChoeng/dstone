	...
	public ToolOutput validateSqlSyntax(@ToolParam(description = "검증할 단일 SQL 문(SELECT/INSERT/UPDATE/DELETE)") String sql) {
		...
		try {
			...
			if (statements.size() != 1) {
				return ToolOutput.fail("한 번에 하나의 SQL 문만 검증할 수 있습니다(세미콜론으로 여러 문장을 이어붙이지 마십시오). 문장 수=" + statements.size());
			}
		} catch (JSQLParserException e) {
			return ToolOutput.fail("문법 오류 - " + e.getMessage());
		} catch (Exception e) {
			return ToolOutput.fail("기타 오류 - " + e.getMessage());
		}
		return ToolOutput.pass("문법 구조상 문제가 없습니다.");
	}
