 ...
 * JSON 스키마를 프롬프트에 자동으로 삽입하고 응답을 파싱해준다 - "통과: .../실패: ..." 같은 텍스트
 * 접두사를 사람이 프롬프트로 지시하고 코드가 문자열 매칭으로 판정하던 예전 방식보다, LLM이 실제 JSON
 * 필드를 채우게 강제하는 쪽이 형식 준수율이 더 높다(그래도 100% 보장은 아니다 - 모델이 스키마 자체를
 * 어기면 파싱 예외가 나는데, 이건 호출부(AgentStepRunner)가 별도로 다룬다).
 *
