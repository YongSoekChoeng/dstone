...
public class WorkFlowExecutor extends BaseObject {
	 ...
	 *
	 * forEach 경로가 가리키는 리스트를 찾아서, 항목마다 {{item}}(또는 itemVariable) 하나만 더한 컨텍스트
	 * 복사본으로 input을 채워 실행합니다. 모든 반복이 끝나면 반복 순서대로 실행 이력을 남기고, 결과를
	 * items에 모읍니다. 반복이 하나라도 실패하면 이 step 전체가 실패입니다. 반복할 항목이 하나도 없으면
	 * 빈 결과로 성공 처리합니다.
	 *
	 * forEach 경로의 값을 찾지 못하거나 그 값이 리스트가 아니면, 이 step은 실패로 처리됩니다(onFailure를 따릅니다).
	 * 반복 하나가 StepRunner 예외(시스템 오류)를 던지면, 그 반복의 실행 이력을 남긴 뒤 예외를 그대로 올려보내서
	 * 실행 전체를 FAILED로 끝냅니다(run()의 3번 설명 참고).
	 *
