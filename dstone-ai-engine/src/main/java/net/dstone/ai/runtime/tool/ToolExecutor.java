...
/**
 * TOOL step이 LLM 없이 Tool 하나를 이름으로 직접 호출하는 유일한 통로다 - 모든 TOOL 호출이 반드시
 * 여길 거치므로, 나중에 governance가 Tool 호출을 감사/제한하고 싶어지면 이 클래스 하나만 감싸면 된다.
 */
@Component
	...
	public String call(String caller, String toolName, String jsonInput) {
		...
		if (callback == null) {
			throw new IllegalStateException(
				"caller[" + caller + "]가 쓸 수 있는 Tool 중 '" + toolName + "'가 없습니다(화이트리스트 또는 이름을 확인하십시오).");
		}
