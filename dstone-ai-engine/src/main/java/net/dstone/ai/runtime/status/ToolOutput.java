 ...
 * 안전합니다. 물론 String을 그냥 그대로 돌려주는 기존 방식의 Tool(예: net.dstone.ai.tools.sql.SqlSyntaxTool)도
 * 여전히 지원됩니다 - 이 경우에는 응답 텍스트가 Constants.Outcome.FAIL_PREFIX("실패")라는 글자로
 * 시작하는지를 보고 판정합니다(success 값이 null이라서, 즉 이 record 형식으로 파싱되지 않을 때는
 * ToolStepRunner가 이 접두사 검사 방식으로 대신 판정합니다). 다만 이 접두사 방식은 Tool을 만드는
 * 사람이 접두사를 빼먹거나 오타를 내도 컴파일할 때 아무 오류 없이 그냥 "성공"으로 조용히 처리되어
 * 버릴 수 있는 위험이 있으므로, 새로 Tool을 만들 때는 이 record를 쓰는 쪽이 더 안전합니다.
 *
