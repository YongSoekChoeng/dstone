 ...
 *
 * 호출하는 쪽은 경우를 따지지 않고 outcome.text()/outcome.output()처럼 같은 방식으로 값을 꺼낼 수 있습니다.
 ...
 *
 * runtime.workflow.WorkFlowExecutor는 이 결과를 컨텍스트의 steps.{stepId}에 {input, output, text, error}로
 * 남기고, 다음 step들은 {{steps.id.text}}, {{steps.id.output.키}}, {{steps.id.error}}로 가져다 씁니다.
 * output은 YAML step의 output(schema/parse)에 선언한 모양의 값이라 이름을 똑같이 맞춰 두었습니다.
 * </pre>
 */
public sealed interface StepOutcome {
	...
	/** 구조화된 결과입니다(steps.id.output). 없으면 빈 맵입니다. */
	 ...
	 */
	record Success(String text, Map<String, Object> output) implements StepOutcome {
	}
	 ...
	 */
	static StepOutcome success(String text, Map<String, Object> output) {
		return new Success(text, output == null ? Map.of() : output);
	}
