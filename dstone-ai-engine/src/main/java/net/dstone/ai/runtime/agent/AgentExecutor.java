...
/**
 * "LLM에게 일을 시키는 단위"인 Agent 하나를 실제로 호출한다. AgentDefinition의 promptName/
 * toolsEnabled/ragEnabled를 읽어 ChatClient 요청을 조립하는 곳은 여기 한 곳뿐이다 -
 * api.controller.ChatController(단일 대화 턴)와 runtime.step.AgentStepRunner(Workflow의 AGENT/
 * SUPERVISOR step)가 똑같이 이 클래스를 통해 호출한다.
 *
 * ragOverride/toolsOverride는 null이면 Agent 정의값을 그대로 쓰고, true/false를 주면 그 호출
 * 한 번만 Agent 정의값을 무시하고 강제로 켜거나 끈다 - api.controller.ChatController가 요청의
 * ChatRequest.ragEnabled()/toolsEnabled()를 그대로 넘겨서, 같은 Agent를 쓰면서도 요청마다 RAG/Tool을
 * 켜고 끄고 싶은 화면(예: dstone-boot의 채팅 화면 체크박스)을 지원한다. Workflow의 AGENT/SUPERVISOR
 * step(runtime.step.AgentStepRunner)은 이 둘을 항상 null로 넘겨 Agent 정의값 그대로 쓴다.
 */
@Component
