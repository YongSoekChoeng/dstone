...
- This environment's actual embedding provider is a locally-installed Ollama (`bge-m3`, 1024 dims) — see `docs/software/14.ollama.md` — because Anthropic has no embeddings API and no OpenAI key is configured here; a real SI deployment can switch to `openai` purely via config.

Phase 3 (Agent/Tool) adds no infrastructure dependency — unlike RAG it's pure Java code executed on demand, so there's no `dstone.ai.*.enabled` gate for it. `common.annotation.AiTool` is a marker annotation (meta-annotated `@Component`) that SI projects put on a class containing `@org.springframework.ai.tool.annotation.Tool`-annotated methods; it lives in `net.dstone.ai.common.annotation` rather than under `agent` because non-agent code (`rag.retrieval.RetrievalTools`) uses it too. `config.ConfigTool` scans for `@AiTool` beans at startup via `ApplicationContext.getBeansWithAnnotation` and wraps them into a `ToolCallbackProvider` (`MethodToolCallbackProvider`) — like the other `Config*` classes, it's `@Import`ed by `config.Config` rather than left to bare component scanning, per the same convention `dstone-boot`/`dstone-batch`/`dstone-batchadmin` use. `api.ChatController` accepts `ChatRequest.toolsEnabled` to attach that provider via `ChatClient`'s `.toolCallbacks(...)`; the actual tool-call loop (LLM requests a tool → it runs → result goes back to the LLM → repeat until a final answer) is handled entirely by Spring AI's `ChatClient` — this is the full extent of the "orchestration" in scope for Phase 3. A sample tool (`agent.tool.sample.DateTimeTools`, `getCurrentDateTime`) ships as a template, mirroring `dstone-boot`'s `sample/` package convention — SI projects replace it with their own domain tools. `rag.retrieval.RetrievalTools` (also `@ConditionalOnProperty(dstone.ai.rag.enabled=true)`) exposes `RetrievalService.search` as a second tool, `searchKnowledgeBase` — this is "agentic RAG": unlike `ChatRequest.ragEnabled` (which unconditionally attaches `QuestionAnswerAdvisor` and always searches), the LLM decides for itself whether a given question warrants a knowledge-base lookup, verified by observing Ollama's `/api/embed` request log line up with domain-specific questions and stay silent for general-knowledge ones.

Phase 5 (Tenant Context) makes `dstone-ai-engine` safe for multiple SI projects (`dstone.boot` apps) to share: the `caller` value `ApiKeyAuthFilter` already resolves from `X-API-Key` and stores in `CallerContext` doubles as a tenant_id, enforced at three points rather than via a new `tenant` package — `capability.CapabilityDefinition.allowedCallers` gates which caller may invoke a given capability (`capability.CapabilityRegistry.resolve(name, caller)`), `common.config.ConfigTool.toolCallbackProvider(caller)` filters the Tool set per caller via `dstone.ai.governance.tool.allowed-by-caller`, and `api.service.RagService` tags every ingested chunk with a `tenant` metadata field and forces a `tenant == caller` filter on search/delete/RAG-augmented chat so one caller's documents never leak into another's results. All three are no-ops when `caller` is null (i.e. `dstone.ai.governance.auth.enabled=false`), so a single-consumer deployment is unaffected. This is logical isolation via a metadata filter, not physical (per-tenant schema/collection) — see `docs/09.dstone-ai-engine.md` §8.6. Capability/Tool whitelisting is verified end-to-end with real Anthropic calls (§8.6); RAG tenant isolation's SQL generation is verified correct (per-caller `WHERE` clause confirmed in logs) but a separate, pre-existing bug — newly-ingested documents never appear in similarity search results regardless of any filter — blocks a full positive-hit proof (§13 troubleshooting table). Verifying this surfaced two real `dstone-common`/`dstone-ai-engine` bugs, now fixed: `ConfigProperty.getListProperty()` bound list elements as `Object` instead of `Map`, failing on any populated `List<Map>` config (not just this feature's); and `CapabilityRegistry` required every capability to have a `promptName`, breaking `processName`-only ones from Phase 6.

Phase 6 (Process Engine, new `process` package) lets a `capability` chain multiple separate prompt calls instead of one: `CapabilityDefinition.processName` (when set) routes `ChatService.chat()` to `process.ProcessExecutor.run()` instead of a single `ChatClient` call — `promptName`/`toolsEnabled`/`ragEnabled` on that capability are then ignored in favor of the `ProcessDefinition`'s own `steps`. There's no new orchestration engine: an `AGENT` step calls `ChatService.runAgentStep()` (the `getSpec()` logic factored out into a primitive `buildSpec()`), a `RAG` step calls `RagService.search()` directly, and a `TOOL` step calls a `ConfigTool`-registered `ToolCallback` without going through the LLM at all — all three keep the Phase 5 tenant gates intact since they're the same underlying calls. `ChatService` and `ProcessExecutor` depend on each other (Process routes to Chat, Chat's AGENT steps call back into Chat) — Spring Boot rejects that cycle by default, so `ProcessExecutor.chatService` is `@Lazy` rather than flipping `spring.main.allow-circular-references` globally. Only 4 control-flow patterns are supported (sequential is the list order; branch/loop via a step's `onSuccess`/`onFailure` pointing at any step id, with `maxIterations` capping total steps executed to stop runaway loops; parallel via adjacent steps sharing a `parallelGroup`) — success/failure for a `TOOL` step is decided by whether the *raw* tool result starts with `"실패"`, reusing the convention `tools.sql.SqlSyntaxTools` already answers with (`AGENT`/`RAG` steps always succeed). Two non-obvious things the live verification caught: Spring AI's `ToolCallback.call()` always JSON-encodes its result (even a plain `String` gets wrapped in quotes), so `ProcessExecutor` unwraps it via Jackson before checking the prefix; and a failed step's forwarded text needs the *original* value plus the failure reason (so a retried step has something to fix), which meant separating success/failure detection from "what gets forwarded" into a `StepResult(success, text)` record rather than overloading the text's own prefix for both. Streaming (`/api/ai/chat/stream`) is not supported for process-backed capabilities (throws `IllegalStateException`) — token-level streaming across multiple steps remains unaddressed even after Phase 10 (which only added async submit/poll, not streaming). Verified end-to-end with a real 4-step Oracle→PostgreSQL run (analyze→convert→validate→review, 3 real Anthropic calls, tool-validated on first try) plus loop/branch/`maxIterations` mechanics on zero-cost pure-`TOOL` test processes — see `docs/09.dstone-ai-engine.md` §15.

Phase 7 (Agent Runtime, new `agent` package) adds no new execution engine: `agent.AgentDefinition`/`AgentRegistry` are `capability`-shaped (name → promptName/toolsEnabled/ragEnabled) but referenced by `ProcessStep.ref` instead of an HTTP request, and `ProcessStepType.SUPERVISOR` reuses the exact `AGENT` call path except its result is judged by the same `"실패"`-prefix convention `TOOL` steps use, rather than always succeeding. `ProcessStep`'s `toolsEnabled`/`ragEnabled` fields moved to `AgentDefinition` in this phase (so multiple processes can share one named Agent) — `ref` now means "Agent name" for `AGENT`/`SUPERVISOR` steps, not "prompt name" directly. Verified by re-running the Phase 6 Oracle→PostgreSQL team through this Agent-Registry-based structure (no regression) and by a minimal draft→judge Supervisor test — which incidentally demonstrated the failure path too: Haiku misjudged an obviously-long draft as "too short" (LLMs are unreliable at precise character counting), and the engine correctly detected the `"실패"` prefix and threw the expected "no onFailure configured" error — a real illustration that Supervisor prompts should ask for qualitative judgment, not precise counting.

Phase 8 (Tool Runtime extension) adds `tools.shell.ShellExecTool`, `tools.python.PythonExecTool`, `tools.http.HttpCallTool` — all default-disabled (`dstone.ai.tool.{shell,python,http}.enabled=false`) and inert until their whitelist is populated, matching every other risky feature's opt-in convention in this module. Shell/Python only ever execute a whitelisted script path (LLM supplies the registered *name* and args, never a raw command string); both share `common.exec.ExternalProcessRunner` (timeout + output-cap wrapper around `ProcessBuilder`, which never invokes a shell, so metacharacters in args can't cause injection). HTTP is GET-only, gated by an exact-match host whitelist (SSRF prevention) rather than a command whitelist. A Database Tool was deliberately not built — this environment's `dstone_ai` DB has no real business tables to scope a whitelist against, the same reason `tools.sql.PostgresExplainValidationTools` was deleted earlier (§7.5). Verified live: whitelisted shell/python scripts execute correctly (confirmed via new `LogUtil.sysout("dstone-ai-engine tool-audit: ...")` lines in each tool, since a successful `TOOL` step forwards the original input rather than the tool's own output — a real limitation for action-style tools, noted in `docs/09.dstone-ai-engine.md` §17.2), a non-whitelisted script name is rejected before any process spawns, and an HTTP GET to an allowed local host succeeds.

Phase 9 completes Governance: a `SensitiveWordGuardrailAdvisor` (`governance.guardrail`, order `HIGHEST_PRECEDENCE+2`) is a near-exact copy of `PiiGuardrailAdvisor` except it matches a configured word list case-insensitively instead of regexes — verified live (mask mode correctly redacted before the text hit Redis chat history, matching PII's proven ordering; reject mode wasn't re-verified live since the code path is identical to PII's already-proven one). Separately, `UsageLoggingAdvisor` gained real budget enforcement (`dstone.ai.governance.usage.quota.*`, `UsageQuotaProperties`) reusing `RateLimitFilter`'s Redis fixed-window counter and `List<Map>` overrides shape, but counting tokens instead of requests: `checkQuota()` blocks *before* the LLM call if the caller's window total already exceeds their limit, and `recordQuotaUsage()` adds the actual tokens used *after* a successful call — this had to be decoupled from `dstone.ai.observability.usage.enabled` (the logging toggle) since quota accounting must keep working even if logging itself is off. Verified live: a caller with a deliberately tiny limit (50 tokens) sailed through their first call (which alone used 11k+ tokens) and was blocked with 429 on the second, while a caller with the default (100k) limit was unaffected in the same run.

Phase 10 (async API) adds `api.controller.ProcessController` (`POST /api/ai/process/submit`, `GET /api/ai/process/status/{jobId}`) and `api.service.ProcessJobService`, using the *same* `ChatRequest` contract as `/api/ai/chat` — it doesn't care whether the target capability is process-backed or a single call, since it just wraps `ChatService.chat()` in `CompletableFuture.runAsync()` (default `ForkJoinPool.commonPool()`, no dedicated executor — an intentional MVP simplification, same spirit as `RateLimitFilter`'s fixed-window counter). Job state lives in a Redis Hash (`dstone:ai:process:job:{jobId}`, 1-hour TTL, fields `status`/`result`/`error`) rather than `opsForValue()`, because `RedisUtil`'s `RedisTemplate` only JSON-serializes hash values (`GenericJackson2JsonRedisSerializer`) — plain values use `StringRedisSerializer`, which can't hold structured data. Verified live end-to-end on both outcomes: a `DONE` job's `result` and a `FAILED` job's `error` (from a deliberately-looping test process hitting `maxIterations`) both populated correctly through the full submit→poll cycle.
