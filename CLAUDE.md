...
| `common.registry` | `WorkFlowRegistry`/`AgentRegistry` — id/name → definition, plus `allowedCallers` (tenant) whitelist check. `WorkFlowRegistry` also validates every step's shape and every `{{ }}` reference at startup and fails boot on the first error |
...
Full architecture, YAML authoring guide, API/config reference, and the live-verification records live in `docs/09.dstone-ai-engine.md`.
