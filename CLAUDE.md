...
| `runtime.tool` | `ToolExecutor` — the one place a `TOOL` step calls a Tool by name without going through the LLM — plus `ToolOutcome`, the structured success/message reply schema a `@Tool` method can return (a tool that wants to emit structured `output` returns any record/Map instead and the YAML step sets `output.parse: json`) |
| `runtime.step` | `StepRunner`'s IN/OUT (`StepInput` — already-rendered `text` or `arguments` plus `workflowInputs`, and `StepOutcome` — a sealed interface: `Success`/`Routed`/`Failure`/`Pending`) alongside its implementations `AgentStepRunner`/`ToolStepRunner`/`ApprovalStepRunner` — per-`StepType` dispatch used by `WorkFlowExecutor` (no `StepType.RAG`/`RagStepRunner` — RAG is either an AGENT's `ragEnabled` advisor or the `RagSearchTool` `@AiTool`) |
...
Step-to-step data flow (redesigned 2026-09-24): every step declares what it takes in `input` (a string for AGENT/SUPERVISOR/ROUTER = the LLM user message, a map for TOOL = the tool arguments; `{{inputs.x}}`/`{{steps.<id>.text|output.<k>|input.<k>|error|items}}`/`{{previous.text}}`/`{{item}}`, `a ?? b` fallback, nothing else) and the shape of what it emits in `output` (AGENT `schema`, TOOL `parse`). All state lives in one execution-context tree (`inputs`/`steps`/`previous`/`approvals`), and its names deliberately match the YAML keys (2026-09-25): `workflow.inputs` → `{{inputs.x}}`, step `input`/`output` → `steps.<id>.input`/`steps.<id>.output` — the old `input.`/`.data` names fail startup with a rename hint; a missing reference fails the step, a runner exception fails the whole run. `${VAR}` in YAML is a separate load-time env substitution. MCP tool calls are serialized per server (`ConfigMcp.SerializedToolCallback`) because one STDIO client can't take concurrent requests. Design record: `docs/temp/dstone-ai-engine-refactory-20260924-1.md`.
